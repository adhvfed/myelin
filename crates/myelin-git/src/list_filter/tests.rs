...
fn not_ids_lowers_to_not_in_over_pr_id() {
    let l = lower_over_pr_id(&SetExpr::NotIds(vec![ObjectId("pr:secret".into())]), &viewer("p:a"));
...
fn in_relation_lowers_to_authz_visible_join_over_pr_id() {
    ...
    let j = &l.joins[0];
    assert!(
        j.clause.contains("JOIN authz_visible av0 ON av0.object_id = pr.id"),
...
fn tuple_set_lowers_to_the_authz_visible_join() {
    let l = lower_over_repo_id(&SetExpr::TupleSet { index: AuthzIndexRef("pull".into()) }, &viewer("p:a"));
...
fn repeated_relation_emits_one_join_no_n_plus_1() {
    let l = lower_over_pr_id(
        &SetExpr::Union(vec![
            SetExpr::InRelation { relation: RelName("view".into()), via_column: pr_id_colref() },
            SetExpr::InRelation { relation: RelName("view".into()), via_column: pr_id_colref() },
        ]),
        &viewer("p:alice"),
    );
    assert_eq!(l.joins.len(), 1, "the same (viewer, relation) JOIN once, however nested — no N+1");
...
fn pr_list_composes_to_one_leak_free_query() {
    ...
    assert_eq!(q.statement_count(), 1, "ONE SQL statement — no N+1, no per-row check loop");
    assert!(q.sql.contains("JOIN authz_visible av0 ON av0.object_id = pr.id"), "{}", q.sql);
    assert!(q.sql.contains("pr.tenant_id = :tenant AND pr.region = :region"), "tenant predicate: {}", q.sql);
    // The ACL predicate is conjoined BEFORE the ORDER BY / LIMIT (pre-filter, never post-filter).
    let acl_pos = q.sql.find("av0.object_id IS NOT NULL").expect("acl predicate present");
    let order_pos = q.sql.find("ORDER BY").expect("order present");
    assert!(acl_pos < order_pos, "the ACL is conjoined BEFORE scoring/pagination (pre-filter): {}", q.sql);
    assert!(q.sql.contains("LIMIT :page"), "the page bound is bound: {}", q.sql);
    assert_eq!(q.filter_mode, FilterMode::PushedDown);
    assert!(q.params.iter().any(|p| p.placeholder == ":tenant" && p.value == "acme"));
    assert!(q.params.iter().any(|p| p.placeholder == ":region" && p.value == "fr-par"));
}
...
fn repo_list_composes_to_one_query_with_tenant_predicate() {
    ...
    assert!(q.sql.contains("repo.tenant_id = :tenant AND repo.region = :region"), "{}", q.sql);
    assert!(q.sql.contains("repo.id IN (:id_0)"), "{}", q.sql);
    assert_eq!(q.filter_mode, FilterMode::Ids, "a materialised Ids allow-set is the Ids mode");
}
...
fn none_set_composes_to_a_false_predicate_no_rows() {
    ...
    assert_eq!(q.statement_count(), 1);
    assert!(q.sql.contains("AND (FALSE)"), "a denied viewer's list is WHERE false (0 rows): {}", q.sql);
}
...
fn code_search_pre_filter_keys_on_repo_over_code_doc_repo_id() {
    ...
    assert!(
        pf.acl_filter.joins[0].clause.contains("av0.object_id = code_doc.repo_id"),
        "the code-search pre-filter keys on the doc's parent-repo id (GIT-P5 acl_object=repo): {}",
        pf.acl_filter.joins[0].clause
    );
    assert!(pf.acl_filter.joins[0].clause.contains("av0.relation = :rel_for_read"));
...
fn partial_pr_visibility_zero_leak_over_the_join() {
    ...
    ix.grant(&tenant(), &region(), "p:alice", "view", "pr:1", "zk-00000000000000000001");
    ix.grant(&tenant(), &region(), "p:alice", "view", "pr:2", "zk-00000000000000000002");
    ix.grant(&tenant(), &region(), "p:bob", "view", "pr:3", "zk-00000000000000000003");

    let set_expr = SetExpr::InRelation { relation: RelName("view".into()), via_column: pr_id_colref() };
    let lowered = lower_over_pr_id(&set_expr, &viewer("p:alice"));
    let candidates = vec![ObjectId("pr:1".into()), ObjectId("pr:2".into()), ObjectId("pr:3".into())];
    let visible = ix.evaluate(&tenant(), &region(), &viewer("p:alice"), &lowered, &candidates);
    assert_eq!(
        visible,
...
fn no_cross_tenant_leak() {
    ...
    let set_expr = SetExpr::InRelation { relation: RelName("view".into()), via_column: pr_id_colref() };
    let lowered = lower_over_pr_id(&set_expr, &viewer("p:alice"));
    let visible = ix.evaluate(&globex, &region(), &viewer("p:alice"), &lowered, &[ObjectId("pr:1".into())]);
    assert!(visible.is_empty(), "an acme grant does not list under globex (no cross-tenant query path)");
}
...
fn difference_all_except_deny_evaluates_correctly() {
    ...
    let visible = ix.evaluate(&tenant(), &region(), &viewer("p:admin"), &lowered, &candidates);
    assert_eq!(visible, vec![ObjectId("repo:a".into())], "admin sees all except the denied repo");
}
...
fn union_of_join_and_ids_evaluates_as_or() {
    let ix = AuthzVisibleIndex::new();
    ix.grant(&tenant(), &region(), "p:alice", "view", "pr:joined", "zk-00000000000000000001");
...
fn just_revoked_grant_drops_out_of_the_list() {
    ...
    ix.revoke(&tenant(), &region(), "p:alice", "view", "pr:1", "zk-00000000000000000003");
    let after = ix.evaluate(&tenant(), &region(), &viewer("p:alice"), &lowered, &candidates);
    assert_eq!(after, vec![ObjectId("pr:2".into())], "the just-revoked pr:1 drops out (zookie reflected)");
}
...
fn new_enemy_guard_serves_at_or_after_falls_back_behind() {
    let ix = AuthzVisibleIndex::new();
    ix.grant(&tenant(), &region(), "p:alice", "view", "pr:1", "zk-00000000000000000005");
    // The watermark is at rev 5.
    assert_eq!(ix.watermark(&tenant(), &region()), Zookie("zk-00000000000000000005".into()));
    // A scan requiring rev 3 (<= 5) → serves.
    assert!(ix.serves(&tenant(), &region(), &Zookie("zk-00000000000000000003".into())));
    // A scan requiring exactly rev 5 → serves (at-or-after is inclusive).
    assert!(ix.serves(&tenant(), &region(), &Zookie("zk-00000000000000000005".into())));
    // A scan requiring rev 7 (> 5) → does NOT serve (fall back to check, never serve stale).
    assert!(!ix.serves(&tenant(), &region(), &Zookie("zk-00000000000000000007".into())));
...
fn watermark_is_monotone_stale_never_regresses() {
    ...
    assert_eq!(ix.watermark(&tenant(), &region()), Zookie("zk-00000000000000000010".into()));
}
...
fn not_ids_deny_set_evaluates_leak_free_over_the_not_in_leaf() {
    ...
    let candidates = vec![ObjectId("repo:visible".into()), ObjectId("repo:denied".into())];
...
fn statement_count_counts_statements_not_constant_one() {
    ...
    assert_eq!(two.statement_count(), 2, "statement_count computes the count, it is not constant 1");
}
...
fn serves_boundary_is_exactly_at_or_after() {
    ...
    assert!(!ix.serves(&tenant(), &region(), &Zookie("zk-00000000000000000006".into())));
}
