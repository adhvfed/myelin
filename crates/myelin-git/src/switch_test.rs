...
mod tests {
    ...
    fn the_switch_test_passes_driven_over_the_real_surface() {
        ...
        // Wall-clock SLA leg: the PR-overview render latency feeds `verdict()` (an over-budget render
        // reds the verdict). Enforced ONLY on the opt-in host/perf lane; in the hermetic
        // `cargo test --lib` lane (debug build under gVisor CPU contention) it is inherently flaky, so
        // clamp it to within-budget BEFORE `verdict()` so contention cannot red the CORRECTNESS
        // verdict; every other leg flows through unchanged. See `myelin_substrate::perf_budget_enforced`.
