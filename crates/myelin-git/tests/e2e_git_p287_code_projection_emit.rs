...
use myelin_git::receive_pack::{
    Oid, ProposedRefUpdate, PushSession, Pusher, QuarantineObject, RefName, RefStore,
    InMemoryObjectDb, CrashPoint, PushOutcome,
};
...
fn push_code_then_projection_emits_per_changed_blob_incrementally() {
    ...
    assert_eq!(store.tip(&RefName::new("refs/heads/main")), Some(Oid::new("commit-1")));
    ...
    let tree1 = Tree::empty()
        .with("src/main.rs", Blob::new("o-main-1", b"fn runServer() {}".to_vec()))
        .with("src/lib.rs", Blob::new("o-lib-1", b"pub fn parse_config() {}".to_vec()))
        .with("README.md", Blob::new("o-readme-1", b"# the project".to_vec()));

    let emit1 = emitter
        .emit_for_push("refs/heads/main", &tip1, &Tree::empty(), &tree1, "initial commit")
        .unwrap()
    ...
    assert_eq!(emit1.emitted.len(), 3, "first index = one blob snapshot per file");
    assert_eq!(cursor.last_indexed("core", "refs/heads/main").as_deref(), Some("commit-1"));
    ...
    let tree2 = Tree::empty()
        ...
        .with("src/main.rs", Blob::new("o-main-2", b"fn runServer() { listen() }".to_vec()))
        // src/lib.rs UNCHANGED (same oid)
        .with("src/lib.rs", Blob::new("o-lib-1", b"pub fn parse_config() {}".to_vec()))
        // README.md deleted (absent)
        // src/net.rs added
        .with("src/net.rs", Blob::new("o-net-1", b"fn connect() {}".to_vec()));
    ...
    assert_eq!(emit2.changed_blob_count, 3, "modified + added + deleted; the unchanged file is skipped");
    assert_eq!(emit2.emitted.len(), 3, "incremental: NOT the whole 3-file tree re-emitted");
    assert_eq!(cursor.last_indexed("core", "refs/heads/main").as_deref(), Some("commit-2"));
    ...
    for id in &emit2.emitted {
        ...
        match pl["op"].as_str().unwrap() {
            "delete" => {
                deletes += 1;
                assert_eq!(pl["path"], serde_json::json!("README.md"), "the deleted file is the tombstone");
            }
            "upsert" => upserts += 1,
            other => panic!("unexpected op {other}"),
        }
    }
    assert_eq!(deletes, 1, "the deleted blob is a tombstone (Gone is never silently dropped)");
    ...
    assert_eq!(emit3.emitted.len(), 0, "a push that changed no blobs emits no projection (incremental)");
    // The cursor still advances to the new tip (the ref moved; the index is up to date).
    assert_eq!(cursor.last_indexed("core", "refs/heads/main").as_deref(), Some("commit-3"));

    // The total outbox: 3 git.ref.updated (one per push) + (3 + 3 + 0) git.blob.snapshot = 9.
    assert_eq!(outbox.committed_count(), 9, "3 ref events + 6 blob snapshots");
...
fn a_feature_branch_push_indexes_no_code() {
    ...
    let push = PushSession {
        ...
        quarantine: vec![QuarantineObject { oid: Oid::new("f1"), bytes: b"x".to_vec() }],
    ...
    let out = emitter
        ...
        .unwrap();
    assert!(out.is_none(), "a feature-branch push does not index code (only indexed refs)");
    // The ref event committed, but no blob snapshot.
    assert_eq!(outbox.committed_count(), 1, "only the git.ref.updated, no projection");
}
