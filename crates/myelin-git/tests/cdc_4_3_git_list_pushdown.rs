...
fn cdc_4_3_identity_filter_lowers_to_one_git_pr_query() {
    ...
    assert_eq!(q.statement_count(), 1, "the consumer composes EXACTLY ONE SQL query (no N+1)");
    assert!(
        q.sql.contains("JOIN authz_visible av0 ON av0.object_id = pr.id"),
...
fn cdc_6_1_code_search_pre_filter_keys_on_repo() {
    ...
    let set_expr = match r {
        ListObjectsResult::Filter { set_expr, .. } => set_expr,
        ListObjectsResult::Ids { .. } => panic!("above the cap the repo read pushes down to Filter"),
    };
    ...
    assert!(pf.acl_filter.joins[0].clause.contains("av0.relation = :rel_for_pull"));
}
...
fn git_d11_chained_grant_list_zero_leak_one_query_then_revoke_reflected() {
    ...
    assert_eq!(q.statement_count(), 1, "ONE SQL query (GIT-D11 signal: 1 query)");
    ...
    for i in 0..VISIBLE {
        av.grant(s.tenant(), &region, "p:viewer", PR_PRODUCER_PERMISSION, &format!("pull_request:pr-{i}"), "zk-00000000000000000001");
    }
    av.grant(s.tenant(), &region, "p:other", PR_PRODUCER_PERMISSION, "pull_request:pr-secret", "zk-00000000000000000001");

    // The candidate universe includes the secret PR (the leak witness).
    let mut candidates: Vec<ObjectId> =
        (0..VISIBLE).map(|i| ObjectId(format!("pull_request:pr-{i}"))).collect();
    ...
    assert_eq!(visible.len(), VISIBLE, "exactly the {VISIBLE} visible PRs survive");
    assert!(
    ...
    av.revoke(s.tenant(), &region, "p:viewer", PR_PRODUCER_PERMISSION, "pull_request:pr-0", "zk-00000000000000000099");
    ...
    assert_eq!(after.len(), VISIBLE - 1, "exactly one PR (the revoked one) dropped out");
    ...
    assert!(
        av.serves(s.tenant(), &region, &Zookie("zk-00000000000000000099".into())),
