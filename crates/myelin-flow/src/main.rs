...
async fn main() {
    ...
    {
        eprintln!(
            "myelin-flow: DATABASE_URL is required (durable-by-default outbox, MR-009b W3b.4): \
    ...
    let config = MyelinConfig::from_env(Mode::DevDefaults).unwrap_or_else(|e| {
        eprintln!("myelin-flow: invalid config: {e}");
        std::process::exit(1);
    });
    let provider = match SubstrateProvider::connect(config, 8).await {
        ...
        Err(e) => {
            eprintln!(
                "myelin-flow: cannot reach the durable OLTP pool (durable-by-default requires PG): {e}"
            );
            std::process::exit(1);
        }
    };
