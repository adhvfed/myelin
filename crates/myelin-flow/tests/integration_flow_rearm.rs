...
async fn flow_p14_cheap_disarm_rearm_is_one_row_update_in_real_postgres() {
    ...
    assert_eq!(
        due_before,
        vec!["sla/issue-7".to_string()],
        "the breach timer is due before the re-arm"
    );

    let new_fire_at: i64 = 28_800;
    let new_bucket: i32 = (new_fire_at / 60) as i32;
    let rearm = format!(
    ...
    let rearm_rows = sqlx::query(&rearm)
        .execute(&admin)
        .await
        .expect("the re-arm")
        .rows_affected();
    assert_eq!(
        rearm_rows, 1,
        "a re-arm is a SINGLE row update (no new row, no wheel rescan)"
    );
    ...
    assert_eq!(
        total, 50_001,
        "the re-arm was an UPDATE - STILL 50_001 rows (no duplicate on the wheel)"
    );
    ...
    let due_after_rearm: Vec<String> = sqlx::query(&due_scan)
        ...
        .collect();
    assert!(
        due_after_rearm.is_empty(),
        "the re-armed breach is no longer due (slid to a far-future bucket)"
    );
    ...
    let disarm_rows = sqlx::query(&disarm)
        .execute(&admin)
        .await
        .expect("the disarm")
        .rows_affected();
    assert_eq!(
        disarm_rows, 1,
        "a disarm is a SINGLE row update (set the partial-index pivot)"
    );
    ...
    let due_after_disarm: Vec<String> = sqlx::query(&due_scan)
        ...
        .collect();
    assert!(
        due_after_disarm.is_empty(),
        "the disarmed timer never fires (the partial index excludes it)"
    );

    let re_disarm = sqlx::query(&disarm)
        .execute(&admin)
        .await
        .unwrap()
        .rows_affected();
    assert_eq!(
        re_disarm, 0,
        "a re-disarm of an already-disarmed timer updates 0 rows"
    );
    ...
    assert_eq!(
        far_unfired, 50_000,
        "the 50k far-future fleet is untouched by the re-arm/disarm (row-op, not wheel-scan)"
    );
