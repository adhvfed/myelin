...
use myelin_notif::{
    ...
    TemplateStore, PLATFORM_DEFAULT_TENANT,
};
...
/// The humanise template key for the **chat HITL-card** string (the in-chat approval card an
...
/// a leaked title (the NOTIF-D4 leak invariant, inherited for free).
...
///
/// The three chat-owned surfaces: the HITL **card** string ([`TPL_CHAT_CARD`]), the **agent-message**
/// string ([`TPL_CHAT_AGENT_MESSAGE`]), and the **`chat.message.mentioned`** notify string
/// ([`TPL_CHAT_MENTIONED`]).
pub fn chat_humanise_templates() -> Vec<HumaniseTemplate> {
    ...
    vec![
        // The in-chat HITL approval card (the `EffectApi` gate's per-effect card). `{0}` is the
        // target artifact; `{1}` (optional) is the effect description. The body is markdown-subset
        // so it round-trips the ONE myelin-content render path; per-viewer-safe by construction.
