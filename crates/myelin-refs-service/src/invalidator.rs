...
use myelin_events::{ArtifactRef, EventEnvelope, EventHandler, HandleOutcome, Reason, SubjectPattern};
...
impl RefsProjectionInvalidator {
    ...
    pub fn with_cache(cache: Arc<dyn ProjectionCache>) -> RefsProjectionInvalidator {
        RefsProjectionInvalidator { cache, busted: Arc::new(AtomicU64::new(0)) }
    }
...
mod tests {
    ...
    fn only_updated_and_erased_are_invalidating() {
        assert!(RefsProjectionInvalidator::is_invalidating("issue.issue.updated"));
        assert!(RefsProjectionInvalidator::is_invalidating("knowledge.page.erased"));
        assert!(RefsProjectionInvalidator::is_invalidating("chat.message.updated"));
        assert!(!RefsProjectionInvalidator::is_invalidating("refs.edge.created"));
        assert!(!RefsProjectionInvalidator::is_invalidating("issue.issue.created"));
        assert!(!RefsProjectionInvalidator::is_invalidating("knowledge.page.removed"));
    }
    ...
    fn updated_event_drives_one_invalidation_per_ref() {
        ...
        assert_eq!(inv.handle(&ev), HandleOutcome::Done, "an *.updated busts the cache");
        let calls = shim.invalidations();
        assert_eq!(calls.len(), 1, "exactly one invalidation call");
        assert_eq!(calls[0].tenant, tenant(), "tenant-first (no cross-tenant bust)");
        assert_eq!(calls[0].region, region());
        assert_eq!(calls[0].ref_.0, ref_, "the exact ArtifactRef the event named");
        assert_eq!(inv.invalidation_count(), 1, "the refs.invalidations telemetry is bumped");
        assert_eq!(
    ...
    fn sub_anchored_update_busts_the_full_ref() {
        ...
        assert_eq!(inv.handle(&ev), HandleOutcome::Done);
        assert_eq!(shim.invalidations()[0].ref_.0, ref_, "the FULL #sub ref is the precise bust key");
    }
    ...
    fn created_event_is_a_noop_no_bust() {
        ...
        assert_eq!(inv.handle(&ev), HandleOutcome::Done, "a created event is handled (no-op)");
    ...
    fn payload_ref_fallback_busts_when_subject_empty() {
        ...
        assert_eq!(inv.handle(&ev), HandleOutcome::Done);
        assert_eq!(shim.invalidations()[0].ref_.0, "myelin://acme/chat/message/m1");
    }
    ...
    fn busts_are_tenant_first_no_cross_tenant_path() {
        ...
        assert_eq!(calls[1].tenant.0, "other", "the second bust is the OTHER tenant — keys never cross");
    }
    ...
    fn shim_records_calls_holds_no_entries() {
        ...
        let inv = RefsProjectionInvalidator::with_cache(Arc::new(shim.clone()));
        inv.handle(&lifecycle_event("01J-u", "issue.issue.updated", "myelin://acme/issue/issue/E1"));
        // The shim recorded the call but holds no projection state (no get/contains — it is no-op).
        assert_eq!(shim.call_count(), 1, "the call was recorded (the WIRING is real)");
    }
}
