...
fn straight_chain_is_walked_with_depths_and_parents() {
    ...
    assert_eq!(ids, vec!["B", "C"], "B (depth 1) then C (depth 2), root excluded");
...
fn rel_filter_does_not_follow_other_relations() {
    ...
    let ids: Vec<&str> = r.nodes.iter().map(|n| n.artifact.0.as_str()).collect();
    assert_eq!(ids, vec!["B"], "only the `blocks` neighbour, never the `mentions` X");
}
...
fn self_referential_graph_terminates_and_reports_cycle() {
    ...
    assert!(r.cycle_detected, "the A→B→A back-edge is surfaced as a cycle DIAGNOSTIC");
    // B is discovered (depth 1); A is NOT re-discovered (the visited-set guard; it is the root).
    let ids: Vec<&str> = r.nodes.iter().map(|n| n.artifact.0.as_str()).collect();
    assert_eq!(ids, vec!["B"], "B once; the root A is never re-expanded (no infinite walk)");
}
...
fn depth_ceiling_truncates_with_marker() {
    ...
    for i in 0..1000 {
        link(&proj, &format!("n{i}"), &format!("n{}", i + 1), "blocks", RelClass::Lifecycle);
    }
    let t = Traverse::with_default_bounds(proj, AuthzVisibleIndex::new());
    assert_eq!(t.depth_ceiling(), TRAVERSE_DEPTH_CEILING, "the seed ceiling is 16");
    ...
    assert!(r.truncated, "a 1000-deep chain under a 16 ceiling is a PARTIAL result");
    ...
    assert_eq!(max_depth, TRAVERSE_DEPTH_CEILING, "the walk stops EXACTLY at depth 16");
    // exactly 16 nodes discovered (n1..n16); n0 is the root.
    assert_eq!(r.nodes.len(), TRAVERSE_DEPTH_CEILING as usize, "16 nodes, never 17");
}
...
fn caller_cannot_exceed_the_ceiling() {
    ...
    let max_depth = r.nodes.iter().map(|n| n.depth).max().unwrap();
    assert!(max_depth <= TRAVERSE_DEPTH_CEILING, "the requested 9999 is clamped to 16");
...
fn an_exhausted_walk_is_not_truncated() {
    ...
    assert!(!r.truncated, "the graph is exhausted before the ceiling → not truncated");
...
fn node_budget_truncates_a_wide_graph() {
    ...
    assert!(r.truncated, "a wide graph past the node budget is a PARTIAL result");
    // visited holds the root + at most (budget-1) neighbours = 8 total.
    assert_eq!(r.visited_count, 8, "the walk visited exactly the node budget, never more");
}
...
fn unreadable_node_is_pruned_no_leak() {
    ...
    let ids: Vec<&str> = r.nodes.iter().map(|n| n.artifact.0.as_str()).collect();
    assert_eq!(ids, vec!["public"], "secret is pruned — the traverse never reveals it");
    assert!(!ids.contains(&"secret"), "0 leak: the unreadable node is absent");
}
...
fn branch_under_an_unreadable_node_is_pruned() {
    ...
    let ids: Vec<&str> = r.nodes.iter().map(|n| n.artifact.0.as_str()).collect();
    assert_eq!(ids, vec!["visible"], "deep is pruned with its unreadable ancestor `secret`");
    assert!(!ids.contains(&"deep"), "0 leak: a node behind an unreadable hop is absent");
...
fn traverse_is_tenant_isolated() {
    ...
    let ids: Vec<&str> = r.nodes.iter().map(|n| n.artifact.0.as_str()).collect();
    assert_eq!(ids, vec!["B"], "only acme's edge; the other tenant's `evil` is invisible");
}
