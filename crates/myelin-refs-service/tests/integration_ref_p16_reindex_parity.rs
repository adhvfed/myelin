...
use myelin_refs_service::{edge_id, RefsReindexSource, SourceEdge, CREATE_EDGE_INDEXES_DDL, CREATE_EDGE_TABLE_DDL, MAKE_EDGE_TENANT_SCOPED_DDL, REFS_OWNER_TOKEN};
...
async fn reindex_from_source_byte_matches_live_on_real_postgres() {
    ...
    truth.record(src_edge("refs.edge:1", "myelin://tenantA/chat/message/m1#block-9", "myelin://tenantA/knowledge/page/7c2#block-3", "embeds", "p-opaque-1"));
    truth.record(src_edge("refs.edge:2", "myelin://tenantA/chat/message/m2", "myelin://tenantA/issue/issue/ENG-1", "mentions", "p-opaque-2"));
    truth.record(src_edge("refs.edge:3", "myelin://tenantA/git/commit/abc", "myelin://tenantA/issue/issue/ENG-2", "links", "p-opaque-1"));
    ...
    async fn rebuild(conn: &mut sqlx::PgConnection, upsert_sql: &str, truth: &RefsReindexSource, scope: &SnapshotScope) {
        for draft in truth.replay(scope, None) {
            ...
            sqlx::query(upsert_sql)
                ...
                .execute(&mut *conn).await.expect("upsert edge");
        }
    }
    ...
    assert_eq!(rebuilt_img, live_img, "the rebuilt edge index byte-matches the live index (cold == live)");
