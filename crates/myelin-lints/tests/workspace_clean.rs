...
const EXCLUDED_SUBSTRINGS: &[&str] = &[
    ...
    "myelin-agent-model/src/",         // the VENDOR-BRAIN crate (AG-P25): the SINGLE sanctioned home for a model SDK / prompt / model-name string (the `no-llm-in-platform` contract-1.6 exception). The whole rest of the platform stays provider-agnostic behind the `AgentRuntime` strategy seam; every provider-specific string (the `gpt-5.6-luna` id, the `api.openai.com` endpoint, `reasoning_effort`, the Luna/Anthropic wire mapping) is QUARANTINED here behind that seam, so swapping Luna→Anthropic→an EU model is a change to this one crate only. A whole-crate boundary BY DESIGN (product-plan §4.1 "the only place a model SDK/prompt may appear"). NAMED, LOUD; the lint stays FULLY live over every other crate — a leak anywhere else still reddens the gate.
