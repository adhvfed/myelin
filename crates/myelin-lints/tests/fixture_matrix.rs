//! THE fixture matrix (the P-S10 → P-017 required test).
//!
//! For each of the four load-bearing lints: run its RED fixture and assert the lint REJECTS it
//! (≥1 violation, fired by THAT lint), run its GREEN fixture and assert the lint ADMITS it (0
//! violations). The full 4×(red-reject + green-admit) matrix is the dated green artifact.
//!
//! Plus the ratchet regression test: removing any one of the four lints' wiring must make the
//! matrix fail (the gate cannot be silently un-wired — EI-01 §5).

use myelin_lints::engine::run;
use myelin_lints::lints::{
    load_bearing_four, no_host_exec, no_raw_publish, no_untagged_personal_data, tenant_predicate,
};
...
#[test]
fn the_matrix_has_a_row_per_load_bearing_lint() {
    // 4/4: the matrix covers exactly the four load-bearing lints, in order.
    let rows = matrix();
    assert_eq!(rows.len(), 4, "the matrix must cover all four load-bearing lints");
    let row_ids: Vec<LintId> = rows.iter().map(|r| r.id).collect();
    let lint_ids: Vec<LintId> = load_bearing_four().iter().map(|l| l.id).collect();
    assert_eq!(row_ids, lint_ids, "matrix rows must match the four lints in order");
}

#[test]
fn every_red_fixture_is_rejected() {
    // 4/4 REJECT: each lint's red fixture produces ≥1 violation, fired by THAT lint.
    for row in matrix() {
        let lint = (row.lint)();
...
fn every_green_fixture_is_admitted() {
    // 4/4 ADMIT: each lint's green fixture produces 0 violations.
    for row in matrix() {
        let lint = (row.lint)();
...
#[test]
fn the_full_four_lint_set_run_rejects_each_red_and_admits_each_green() {
    // The set-level gate (the form CI runs): run() over ALL four lints is Err on each red
    // fixture and Ok on each green fixture. This is the loud-never-swallowed surface.
    let all = load_bearing_four();
    for row in matrix() {
        assert!(
            run(&all, &read_fixture(row.red)).is_err(),
            "the four-lint set must REJECT red fixture `{}`",
            row.red
        );
        assert!(
            run(&all, &read_fixture(row.green)).is_ok(),
            "the four-lint set must ADMIT green fixture `{}`",
            row.green
        );
    }
}
...
fn removing_any_lint_breaks_the_matrix() {
    // THE RATCHET REGRESSION TEST: if any one of the four lints is un-wired, that lint's red
    ...
    for (drop_idx, dropped) in rows.iter().enumerate() {
        ...
        let reduced: Vec<Lint> = load_bearing_four()
            .into_iter()
            .enumerate()
        ...
        // one lint, so dropping that lint makes it pass.)
        let full_violations = run(&load_bearing_four(), &red);
