...
fn record_stores_one_marker_per_subject_root_zero_write_amplification() {
    ...
    for i in 0..1000 {
        store.record(&tenant(), &subj, crate::Reason::Watched, &origin(&format!("e{i}")));
    }
    ...
    let m = store.get(&tenant(), "myelin://acme/chat/thread/celebrity").unwrap();
    assert_eq!(m.count, 1000, "the +N more counter accumulated the full activity (preserved, never lost)");
...
fn record_coalesces_subthreads_into_the_root_marker() {
    let store = AmbientMarkerStore::new();
    store.record(&tenant(), &subject("myelin://acme/chat/thread/T1#c1"), crate::Reason::Watched, &origin("a"));
    store.record(&tenant(), &subject("myelin://acme/chat/thread/T1#c2"), crate::Reason::Watched, &origin("b"));
    assert_eq!(store.marker_count(&tenant()), 1, "sub-thread activity coalesces into the ONE root marker");
    let m = store.get(&tenant(), "myelin://acme/chat/thread/T1").unwrap();
    assert_eq!(m.count, 2);
}
...
fn markers_are_tenant_partitioned() {
    let store = AmbientMarkerStore::new();
    store.record(&tenant(), &subject("myelin://acme/git/pr/9"), crate::Reason::Watched, &origin("a"));
    let other = TenantId("globex".into());
    assert_eq!(store.marker_count(&tenant()), 1);
    assert_eq!(store.marker_count(&other), 0, "another tenant's read sees 0 markers (partition key)");
...
fn seeded() -> (AmbientMarkerStore, SyntheticReverseIndex) {
    let store = AmbientMarkerStore::new();
    store.record(&tenant(), &subject("root/a"), crate::Reason::Watched, &origin("a"));
    store.record(&tenant(), &subject("root/b"), crate::Reason::Watched, &origin("b"));
    store.record(&tenant(), &subject("root/c"), crate::Reason::Watched, &origin("c"));
...
fn read_fanout_materialises_only_the_watched_slice_via_the_join() {
    ...
    assert_eq!(roots, vec!["root/a", "root/c"], "the viewer sees exactly the roots they watch (a, c), stable order");
    // the un-watched root b is NOT in the slice (the JOIN excluded it — not a post-filter over all).
    assert!(!roots.contains(&"root/b"), "a non-watched subject's marker is NOT in the viewer's slice");
}
...
fn read_fanout_for_a_non_watcher_is_empty() {
    ...
    assert!(slice.is_empty(), "a principal who watches nothing materialises 0 ambient markers");
}
...
fn setexpr_lowering_covers_every_algebra_arm() {
    ...
    assert_eq!(roots(SetExpr::All).len(), 4, "All → every marker (type+tenant scoped)");
    // None → ∅ (WHERE false) — the deny short-circuit.
    assert!(roots(SetExpr::None).is_empty(), "None → ∅ (WHERE false)");
    // Ids → exactly the listed roots.
    assert_eq!(roots(SetExpr::Ids(vec![id("root/a"), id("root/c")])), vec!["root/a", "root/c"]);
    // NotIds → every root EXCEPT the deny-set.
    assert_eq!(roots(SetExpr::NotIds(vec![id("root/a")])), vec!["root/b", "root/c", "root/d"]);
    // Union(Ids{a}, Ids{b}) → a ∪ b.
    assert_eq!(
        roots(SetExpr::Union(vec![SetExpr::Ids(vec![id("root/a")]), SetExpr::Ids(vec![id("root/b")])])),
        vec!["root/a", "root/b"]
    );
...
fn tupleset_relational_form_resolves_via_the_join() {
    ...
    impl WatcherResolvePort for TupleSetPort {
        fn list_objects(
            ...
            Ok(myelin_identity::ListObjectsResult::Filter {
                set_expr: SetExpr::TupleSet { index: AuthzIndexRef("watcher-idx".into()) },
    ...
    let roots: Vec<&str> = slice.iter().map(|m| m.subject_root.as_str()).collect();
    assert_eq!(roots, vec!["root/a", "root/c"], "the TupleSet big-result path resolves the same watched slice");
}
...
fn revoked_watch_is_reflected_held_not_leaked() {
    ...
    let before = read_fanout(&viewer("u1"), &store, &idx, &strong(&idx.current_zookie().0)).unwrap();
    let before_roots: Vec<&str> = before.iter().map(|m| m.subject_root.as_str()).collect();
    assert_eq!(before_roots, vec!["root/a", "root/c"], "before revoke: u1 sees a + c");
    ...
    assert_eq!(after_roots, vec!["root/a"], "after revoke: the revoked subject c is ABSENT (held, not leaked)");
    assert!(!after_roots.contains(&"root/c"), "0 leaked item on a revoked watch (contract 4.10)");
}
...
fn stale_reverse_index_revision_is_rejected_not_served() {
    ...
    match err {
        ReadFanoutError::StaleReverseIndex { required, served } => {
            assert!(served < required, "the served revision is below the required watermark (rejected, not served)");
        }
...
fn unavailable_resolver_holds_not_leaks() {
    ...
    let err = read_fanout(&viewer("u1"), &store, &idx, &strong(&idx.current_zookie().0)).unwrap_err();
    assert!(matches!(err, ReadFanoutError::Unavailable(_)), "an Id hiccup is held, not leaked (never fall open)");
}
...
fn bounded_stale_read_serves_from_a_lower_watermark() {
    ...
    let bounded = Consistency { at_least: current, mode: ConsistencyMode::BoundedStale };
    ...
    let roots: Vec<&str> = slice.iter().map(|m| m.subject_root.as_str()).collect();
    assert_eq!(roots, vec!["root/a", "root/c"], "a BoundedStale read serves from the lower watermark (no reject)");
}
...
fn ids_materialised_path_needs_no_join() {
    ...
    assert_eq!(slice.len(), 1, "the bounded Ids watch set materialised directly (S4, no JOIN)");
...
fn honours_is_revision_ge_watermark() {
    let answer = |rev: u64| ReverseIndexAnswer { subject_roots: BTreeSet::new(), revision: RevisionWatermark(rev) };
    assert!(answer(5).honours(RevisionWatermark(5)), "revision == watermark honours (the boundary)");
    assert!(answer(6).honours(RevisionWatermark(5)), "a newer revision honours");
    assert!(!answer(4).honours(RevisionWatermark(5)), "a revision below the watermark does NOT honour (rejected)");
}
...
fn default_resolve_relation_is_unavailable() {
    ...
    assert!(matches!(err, ReadFanoutError::Unavailable(_)), "no reverse index → held, not leaked (default unavailable)");
}
...
fn synthetic_revoke_bumps_the_revision_strictly() {
    ...
    assert!(after_revoke > after_grant, "revoke advances the monotone revision strictly (a newer zookie)");
    ...
    assert!(after_second > after_revoke, "every revoke advances the revision (a mutant flattening it is caught)");
}
...
fn synthetic_serves_only_the_watcher_relation() {
    ...
    let watched = idx.resolve_relation(&viewer("u1"), &watcher_leaf, req).unwrap();
    assert_eq!(watched.subject_roots.len(), 1, "the watcher relation resolves the watched set");
    ...
    let other = idx.resolve_relation(&viewer("u1"), &other_leaf, req).unwrap();
    assert!(other.subject_roots.is_empty(), "a non-watcher relation resolves to ∅ (the relation guard bites)");
}
...
fn marker_carries_refs_not_payloads() {
    let store = AmbientMarkerStore::new();
    store.record(&tenant(), &subject("myelin://acme/git/pr/9"), crate::Reason::Watched, &origin("e1"));
    let m = store.get(&tenant(), "myelin://acme/git/pr/9").unwrap();
    assert_eq!(m.subject, ArtifactRef("myelin://acme/git/pr/9".into()), "the subject is a ref, never a payload");
    assert_eq!(m.latest_origin, origin("e1"), "the origin is a ref (the NOTIF-2 provenance)");
}
