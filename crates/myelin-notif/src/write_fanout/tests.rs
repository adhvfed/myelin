...
fn extract_mentions_reads_only_the_structured_mention_node() {
    ...
    assert_eq!(mentions.len(), 2, "exactly the two Mention nodes (ref/embed are not mentions)");
...
fn extract_mentions_no_structured_node_no_fanout() {
    assert!(extract_mentions(&[]).is_empty(), "an empty body → no mentions");
    let only_refs = vec![
...
fn extract_mentions_dedups_the_same_principal_in_one_body() {
    ...
    let mentions = extract_mentions(&nodes);
    assert_eq!(mentions.len(), 2, "alice twice → one recipient (deduped); bob → one");
...
fn hot_cap_admits_within_the_cap() {
    ...
    assert_eq!(cap.admit("p-c", root), CapVerdict::Admit, "3rd within cap (== cap)");
    assert_eq!(cap.admitted_count(root), 3, "three distinct recipients admitted");
    assert_eq!(cap.overflow_count(root), 0, "none overflowed within the cap");
}
...
fn hot_cap_overflows_a_storm_past_the_cap_bounded() {
    ...
    assert_eq!(admitted, 3, "exactly `cap` rows materialise (bounded write-fanout)");
    ...
    assert_eq!(cap.overflow_count(root), 7, "the overflow count is preserved (bounded, not lost)");
}
...
fn hot_cap_repeat_recipient_does_not_consume_a_fresh_slot() {
    ...
    for _ in 0..100 {
        assert_eq!(cap.admit("p-a", root), CapVerdict::Admit, "a repeat re-admits, no new slot");
    }
    assert_eq!(cap.admitted_count(root), 1, "still ONE distinct admitted recipient");
    // A genuinely NEW recipient still fits the cap (the repeats did not exhaust it).
    assert_eq!(cap.admit("p-b", root), CapVerdict::Admit, "a new recipient still fits (cap not eaten)");
    assert_eq!(cap.admit("p-c", root), CapVerdict::Overflow, "now past cap-2 → overflow");
}
...
fn hot_cap_is_per_subject_root() {
    ...
    assert_eq!(cap.admit("p-a", a), CapVerdict::Admit, "A's first fits A's cap");
    assert_eq!(cap.admit("p-b", a), CapVerdict::Overflow, "A's second overflows A's cap-1");
    assert_eq!(cap.admit("p-a", b), CapVerdict::Admit, "B has its OWN cap (independent budget)");
...
fn default_cap_is_the_frozen_floor() {
    ...
    assert_eq!(HotSubjectCap::new().cap(), 64, "the default constructor uses the frozen floor");
