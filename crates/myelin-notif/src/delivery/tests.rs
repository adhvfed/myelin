...
fn in_app_is_the_only_in_cell_channel() {
    assert!(Channel::InApp.is_in_cell(), "in_app is the in-cell inbox push (never egresses)");
    assert!(!Channel::InApp.is_off_cell());
    // Every other channel egresses → MUST be redacted.
    for c in [Channel::WebPush, Channel::MobilePush, Channel::Email, Channel::Desktop] {
        assert!(c.is_off_cell(), "{} egresses → off-cell, redacted", c.token());
        assert!(!c.is_in_cell());
    }
}
...
fn idem_key_is_stable_per_item_channel_and_channel_scoped() {
    ...
    assert_eq!(build_idem_key("itm-1", Channel::Email), build_idem_key("itm-1", Channel::Email));
    ...
    assert_ne!(build_idem_key("itm-1", Channel::Email), build_idem_key("itm-1", Channel::WebPush));
    // Item-scoped: different items never collapse.
    assert_ne!(build_idem_key("itm-1", Channel::Email), build_idem_key("itm-2", Channel::Email));
}
...
fn redact_for_offcell_carries_summary_and_link_not_the_body() {
    ...
    assert_eq!(msg.rendered.links, vec!["myelin://acme/issues/issue/PROJ-1".to_string()]);
...
fn ledger_record_is_first_writer_wins_on_idem_key() {
    ...
    assert!(!ledger.record(&tenant(), rec(false)), "the duplicate is collapsed (no-op)");
    assert!(ledger.get(&tenant(), "itm-1:email").unwrap().accepted, "the first row is preserved");
...
fn mock_adapter_records_each_send_and_accepts_by_default() {
    ...
    assert_eq!(mock.send_count("itm-1:email"), 1, "the provider was invoked once");
...
fn deliver_is_idempotent_a_retry_after_ack_is_a_noop() {
    let ledger = DeliveryLedger::new();
    let mock = MockAdapter::new(Channel::Email, region());
    let fabric = DeliveryFabric::new(ledger.clone()).with_adapter(std::sync::Arc::new(mock.clone()));
    let msg = redacted_msg();

    // First delivery → the provider is invoked, the ledger row recorded, exactly one effective.
    let out = fabric.deliver(&tenant(), "itm-1", Channel::Email, &msg).unwrap();
    assert!(matches!(out, DeliveryOutcome::Delivered(_)), "first deliver is a new effective delivery");
    assert_eq!(mock.send_count("itm-1:email"), 1, "provider invoked exactly once");
    ...
    assert_eq!(mock.send_count("itm-1:email"), 1, "the provider was NOT invoked again (exactly once)");

    // Exactly ONE effective delivery per (item, channel) — the NOTIF-D9 threshold (never 2).
    assert_eq!(effective_delivery_count(&ledger, &tenant(), "itm-1", Channel::Email), 1);
...
fn deliver_records_redacted_true_for_offcell_and_false_for_in_app() {
    ...
    fabric.deliver(&tenant(), "itm-1", Channel::Email, &redacted_msg()).unwrap();
    let email_row = ledger.get(&tenant(), "itm-1:email").unwrap();
    assert!(email_row.redacted, "off-cell email is redacted (delivery.redacted=true)");

    // In-app → redacted=false (stays in-cell; no off-cell egress, no redaction flag).
    let in_app_msg = RedactedMessage { rendered: summary(), class: Class::Direct };
    fabric.deliver(&tenant(), "itm-1", Channel::InApp, &in_app_msg).unwrap();
    let in_app_row = ledger.get(&tenant(), "itm-1:in_app").unwrap();
    assert!(!in_app_row.redacted, "in_app stays in-cell (redacted=false, no off-cell egress)");
}
...
fn deliver_to_two_channels_is_two_distinct_deliveries() {
    ...
    let fabric = DeliveryFabric::with_mock(ledger.clone(), region());
    fabric.deliver(&tenant(), "itm-1", Channel::Email, &redacted_msg()).unwrap();
    fabric.deliver(&tenant(), "itm-1", Channel::WebPush, &redacted_msg()).unwrap();
    ...
    assert_eq!(effective_delivery_count(&ledger, &tenant(), "itm-1", Channel::Email), 1);
    assert_eq!(effective_delivery_count(&ledger, &tenant(), "itm-1", Channel::WebPush), 1);
}
...
fn deliver_a_bounce_is_recorded_and_still_idempotent() {
    ...
    let out = fabric.deliver(&tenant(), "itm-1", Channel::Email, &redacted_msg()).unwrap();
    assert!(matches!(out, DeliveryOutcome::Bounced(_)), "a rejected message is a bounce");
    // A bounce IS recorded (so a blind retry does not re-deliver) — still exactly one effective row.
    assert_eq!(ledger.effective_count(&tenant()), 1);
    let retry = fabric.deliver(&tenant(), "itm-1", Channel::Email, &redacted_msg()).unwrap();
    assert_eq!(retry, DeliveryOutcome::AlreadyDelivered { accepted: false });
    assert_eq!(mock.send_count("itm-1:email"), 1, "the bounced send is not re-invoked on retry");
}
...
fn deliver_to_an_unregistered_channel_errors_loudly() {
    ...
    let fabric = DeliveryFabric::new(ledger)
        .with_adapter(std::sync::Arc::new(MockAdapter::new(Channel::Email, region())));
    let err = fabric.deliver(&tenant(), "itm-1", Channel::WebPush, &redacted_msg()).unwrap_err();
    assert_eq!(err, DeliveryError::NoAdapter("web_push"), "no adapter → loud error, no silent drop");
...
fn eu_preferring_posture_recognises_eu_regions() {
    assert!(is_eu_region(&Region("fr-par".into())), "fr-par (Scaleway Paris) is EU");
    ...
    assert!(!is_eu_region(&Region("us-east".into())), "us-east is NOT EU");
    ...
    assert!(is_eu_region(mock.region()), "the mock delivers from an EU region (EU-preferring)");
}
...
fn channel_token_round_trips() {
    for c in [
        ...
        assert_eq!(channel_from_token(c.token()), Some(c), "{} round-trips", c.token());
    }
    assert_eq!(channel_from_token("not-a-channel"), None, "an unknown token is None");
}
...
fn effective_delivery_count_is_zero_for_an_undelivered_item() {
    ...
    assert_eq!(effective_delivery_count(&ledger, &tenant(), "never", Channel::Email), 0);
    // And it reads exactly 1 once delivered (never 2 — the per-key boolean count).
    let fabric = DeliveryFabric::with_mock(ledger.clone(), region());
    fabric.deliver(&tenant(), "itm-1", Channel::Email, &redacted_msg()).unwrap();
    assert_eq!(effective_delivery_count(&ledger, &tenant(), "itm-1", Channel::Email), 1);
    assert_eq!(effective_delivery_count(&ledger, &tenant(), "itm-1", Channel::WebPush), 0);
}
...
fn with_mock_registers_an_adapter_for_every_channel() {
    ...
    for c in [
        ...
        assert!(fabric.adapter(c).is_some(), "the mock fabric has an adapter for {}", c.token());
        assert!(is_eu_region(fabric.adapter(c).unwrap().region()), "EU-preferring per channel");
    }
}
