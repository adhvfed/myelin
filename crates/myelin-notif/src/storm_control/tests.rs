...
fn mechanism_1_self_notification_is_suppressed() {
    ...
    let d = sc.decide(&env, &it, "myelin://acme/chat/thread/T1", false, &ctx(0, &q));
    ...
    assert!(!d.touches_audit(), "but the underlying event is untouched on the bus");
}
...
fn mechanism_1_notification_to_another_principal_is_not_suppressed() {
    ...
    let d = sc.decide(&env, &it, "myelin://acme/chat/thread/T1", false, &ctx(0, &q));
    assert_eq!(d, StormDecision::Deliver, "a notification to a DIFFERENT principal delivers");
}
...
fn is_self_notification_matches_actor_principal_exactly() {
    assert!(is_self_notification(&env_by("p-x"), "p-x"), "actor == recipient → self");
    assert!(!is_self_notification(&env_by("p-x"), "p-y"), "actor != recipient → not self");
}
...
fn mechanism_2_existing_row_collapses() {
    ...
    assert_eq!(second, StormDecision::Collapse, "a same-key candidate collapses into the existing row");
    assert!(second.writes_row(), "a collapse touches the existing row (coalesce_count++)");
    assert!(!second.delivers(), "a collapse does not re-push (the +N more is read at inbox open)");
...
fn mechanism_3_second_participating_item_coalesces() {
    ...
    let a = item("p-bob", "myelin://acme/chat/thread/T1#c1", Class::Participating);
    let b = item("p-bob", "myelin://acme/chat/thread/T1#c2", Class::Participating);
    let root = "myelin://acme/chat/thread/T1";
    let first = sc.decide(&env, &a, root, false, &ctx(0, &q));
    assert_eq!(first, StormDecision::Deliver, "the first opens the digest marker (delivers)");
    let second = sc.decide(&env, &b, root, false, &ctx(0, &q));
    assert_eq!(second, StormDecision::Coalesce, "the second participating item coalesces (digest)");
    assert!(second.writes_row(), "coalescing touches the digest marker row");
    assert!(!second.delivers(), "a coalesced item does not push a new delivery");
}
...
fn mechanism_3_direct_item_is_broken_out_never_coalesced() {
    ...
    sc.decide(&env, &item("p-bob", "myelin://acme/issues/issue/PROJ-1#a", Class::Participating), root, false, &ctx(0, &q));
    // A DIRECT item on the same root is broken out (delivers, not coalesced).
    let direct = item("p-bob", "myelin://acme/issues/issue/PROJ-1#b", Class::Direct);
    let d = sc.decide(&env, &direct, root, false, &ctx(0, &q));
    assert_eq!(d, StormDecision::Deliver, "a Direct item is broken out (never digested)");
    // A CRITICAL item likewise.
    let crit = item("p-bob", "myelin://acme/issues/issue/PROJ-1#c", Class::Critical);
    let dc = sc.decide(&env, &crit, root, false, &ctx(0, &q));
    assert_eq!(dc, StormDecision::Deliver, "a Critical page is broken out (never digested)");
}
...
fn coalescer_break_out_vs_digest() {
    ...
    assert!(!c.should_coalesce("r", "root", Class::Participating), "first ambient opens the marker");
    assert!(c.should_coalesce("r", "root", Class::Watching), "second ambient coalesces");
    // A DIFFERENT root has its own marker.
    assert!(!c.should_coalesce("r", "other", Class::Fyi), "a different root opens its own marker");
}
...
fn mechanism_4_token_bucket_burst_then_damp_then_refill() {
    ...
    let cfg = RateConfig { capacity: 3.0, refill_per_tick: 1.0 };
    ...
    assert!(!tb.try_take("r", "root", 0, cfg), "the 4th in the burst is damped (bucket empty)");
    // One tick later refills one token → one admits, then damped again.
    assert!(tb.try_take("r", "root", 1, cfg), "one tick refilled one token");
    assert!(!tb.try_take("r", "root", 1, cfg), "and the bucket is empty again");
    // A DIFFERENT (recipient, subject_root) has its OWN full bucket.
    assert!(tb.try_take("r", "other", 0, cfg), "a different root has its own burst allowance");
}
...
fn token_bucket_multi_tick_refill_is_elapsed_times_rate_capped() {
    let tb = TokenBucket::new();
    let cfg = RateConfig { capacity: 6.0, refill_per_tick: 2.0 };
    // Deplete the full burst (6) at tick 0.
    for _ in 0..6 {
        assert!(tb.try_take("r", "root", 0, cfg));
    }
    assert!(!tb.try_take("r", "root", 0, cfg), "depleted: same-tick take is damped (no refill at elapsed 0)");
    // Advance 2 ticks → refill `2 * 2 = 4` tokens (NOT `2 / 2 = 1`). Exactly 4 admit, then damp.
    for k in 0..4 {
        assert!(tb.try_take("r", "root", 2, cfg), "tick+2 refilled 4 tokens (2*2): admit {k}");
    }
    assert!(!tb.try_take("r", "root", 2, cfg), "after the 4 refilled tokens the bucket is empty");
    // Advance far → the refill is CAPPED at capacity (6), never unbounded.
    for k in 0..6 {
        assert!(tb.try_take("r", "root", 100, cfg), "a long elapse refills to the capacity cap (6): admit {k}");
    }
    assert!(!tb.try_take("r", "root", 100, cfg), "the refill is capped at capacity (not 98*2)");
}
...
fn mechanism_4_burst_on_hot_subject_is_rate_damped() {
    ...
    let mut damped = 0;
    for i in 0..10 {
        let it = item("p-bob", &format!("myelin://acme/git/pr/9#c{i}"), Class::Direct);
    ...
    assert_eq!(damped, 5, "the rest of the 10 were rate-damped (bounded, not a flood)");
}
...
fn mechanism_4_critical_page_is_never_rate_damped() {
    ...
    for i in 0..100 {
        let it = item("p-oncall", &format!("myelin://acme/oncall/page/1#x{i}"), Class::Critical);
        let d = sc.decide(&env, &it, root, false, &ctx(0, &q));
        assert_eq!(d, StormDecision::Deliver, "a critical page is never damped (iter {i})");
    }
}
...
fn mechanism_5_muted_thread_suppresses_delivery_but_writes_the_row() {
    ...
    let it = item("p-bob", "myelin://acme/chat/thread/T9#c1", Class::Participating);
    let d = sc.decide(&env, &it, root, false, &ctx(0, &q));
    assert_eq!(d, StormDecision::Suppress(SuppressReason::Muted));
    assert!(d.writes_row(), "a muted item STILL writes the inbox row (the ONE inbox receives)");
...
fn mechanism_5_quiet_hours_suppresses_non_piercing_pierces_critical() {
    ...
    assert_eq!(d, StormDecision::Suppress(SuppressReason::QuietHours), "a non-piercing item is quiet-suppressed");
    assert!(d.writes_row(), "the row is still written (only the push is suppressed)");

    // A CRITICAL item pierces the same window.
    let crit = item("p-bob", "myelin://acme/issues/issue/PROJ-2#b", Class::Critical);
    let dc = sc.decide(&env, &crit, root, false, &in_window);
    assert_eq!(dc, StormDecision::Deliver, "a critical item pierces quiet-hours (you cannot silence a page)");
}
...
fn ordering_self_suppression_wins_over_all_others() {
    ...
    let quiet = QuietHours {
        tz: Tz::UTC,
        windows: vec![QuietWindow { from: 0, to: 1440, days: vec![] }], // always quiet
        pierce_classes: vec![],
    };
    let env = env_by("p-alice");
    let it = item("p-alice", "myelin://acme/chat/thread/T1#c1", Class::Fyi);
    let cx = StormContext { tick: 0, utc_minute_of_day: 0, utc_weekday: 0, quiet: &quiet, rate: RateConfig::default() };
    assert_eq!(
...
fn ordering_dedup_collapse_wins_over_mute_and_quiet() {
    ...
    assert_eq!(d, StormDecision::Collapse, "an existing row collapses (mechanism 2) before mute (5)");
}
...
fn every_verdict_leaves_the_audit_untouched() {
    ...
    for v in verdicts {
        assert!(!v.touches_audit(), "{v:?} must NOT touch the audit (EI-04 §5.3)");
    }
}
...
fn suppress_reason_writes_row_mapping_is_frozen() {
    assert!(!SuppressReason::SelfAction.writes_row(), "self-action writes no row");
    assert!(!SuppressReason::RateDamped.writes_row(), "rate-damp writes no row");
    assert!(SuppressReason::Muted.writes_row(), "mute writes the row (only push suppressed)");
    assert!(SuppressReason::QuietHours.writes_row(), "quiet-hours writes the row (only push suppressed)");
...
fn subject_root_strips_sub_fragment() {
    assert_eq!(subject_root_of("myelin://acme/chat/thread/T1#c5"), "myelin://acme/chat/thread/T1");
    assert_eq!(subject_root_of("myelin://acme/ci/run/42"), "myelin://acme/ci/run/42", "no fragment → identity");
...
fn dedup_collapse_ratio_basis_points() {
    assert_eq!(dedup_collapse_ratio_bps(0, 0), 0, "no inbound → 0 (nothing to measure)");
    assert_eq!(dedup_collapse_ratio_bps(1000, 999), 9990, "1000 → 1 row: 999 collapsed → 9990 bps");
    assert_eq!(dedup_collapse_ratio_bps(2, 1), 5000, "2 → 1 row: 1 collapsed → 5000 bps (50%)");
    assert_eq!(dedup_collapse_ratio_bps(10, 0), 0, "no collapse → 0 bps");
    assert_eq!(dedup_collapse_ratio_bps(5, 100), 10000, "collapsed capped at inbound → 10000 bps");
}
...
fn notif_d2_chained_storm_collapses_to_one_row_ratio_measured() {
    ...
    assert_eq!(rows, 1, "NOTIF-D2: 1000 near-identical CI failures → ONE inbox row (N→1)");
    assert_eq!(collapsed, 999, "the other 999 collapsed (coalesce_count would be 1000)");
    ...
    assert_eq!(ratio, 9990, "the dedup-collapse-ratio is ~99.9% (the storm was absorbed write-time)");
    assert!(ratio >= 9000, "ratio floor: a regression that stops collapsing fails this LOUDLY");
}
...
fn notif_d2_chained_self_burst_produces_zero_items() {
    ...
    for i in 0..30 {
        let it = item("p-bob", &format!("myelin://acme/chat/thread/T1#c{i}"), Class::Participating);
        let d = sc.decide(&env, &it, root, false, &ctx(0, &q));
        assert_eq!(d, StormDecision::Suppress(SuppressReason::SelfAction));
        assert!(!d.writes_row() && !d.delivers());
        self_suppressed += 1;
    }
    assert_eq!(self_suppressed, 30, "NOTIF-D2: a 30-event self-burst → 0 inbox items (0 self-notifications)");
}
...
fn notif_d2_chained_30_comment_pr_burst_is_bounded_audit_untouched() {
    ...
    for i in 0..30 {
        ...
        let it = item("p-watcher", &format!("myelin://acme/git/pr/9#c{i}"), Class::Participating);
        let d = sc.decide(&env, &it, root, false, &ctx(0, &q));
        assert!(!d.touches_audit(), "no verdict touches the audit (EI-04 §5.3)");
        match d {
            StormDecision::Deliver => delivered += 1,
    ...
    assert_eq!(delivered, 1, "the FIRST comment delivers (opens the digest marker)");
    assert_eq!(bounded, 29, "the other 29 are bounded (coalesced/damped) — not a flood");
}
