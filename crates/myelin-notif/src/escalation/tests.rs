...
fn schedule() -> OncallSchedule {
    OncallSchedule {
        ...
        rotation: vec![
            ...
            RotationWindow { from_minute: 0, to_minute: 720, principal: pid("psn:alice") },
            RotationWindow { from_minute: 720, to_minute: 1440, principal: pid("psn:bob") },
        ],
    }
}
...
fn oncall_now_first_covering_window_wins_override_semantics() {
    let s = OncallSchedule {
        ...
        rotation: vec![
            ...
            RotationWindow { from_minute: 480, to_minute: 540, principal: pid("psn:carol") },
            RotationWindow { from_minute: 0, to_minute: 1440, principal: pid("psn:dave") },
        ],
    };
    assert_eq!(oncall_now(&s, 500), Some(pid("psn:carol")), "the override window wins inside it");
    assert_eq!(oncall_now(&s, 600), Some(pid("psn:dave")), "the base window outside the override");
}
...
fn oncall_now_uncovered_instant_is_none_not_silent_drop() {
    let s = OncallSchedule {
        ...
        rotation: vec![RotationWindow { from_minute: 0, to_minute: 60, principal: pid("psn:eve") }],
    };
...
fn step_at_walks_the_steps_in_order() {
    ...
    assert_eq!(p.step_at(0).unwrap().target, EscalationTarget::Schedule("platform-oncall".into()));
    assert_eq!(p.step_at(1).unwrap().target, EscalationTarget::Principal(pid("psn:lead")));
}
...
fn step_at_exhausts_after_the_last_step_when_repeat_one() {
    ...
    assert!(p.step_at(2).is_none(), "walk 2 is exhausted (gave up after the chain walked once)");
}
...
fn step_at_repeat_wraps_the_chain() {
    ...
    assert_eq!(p.step_at(2).unwrap().target, EscalationTarget::Schedule("platform-oncall".into()));
    assert_eq!(p.step_at(3).unwrap().target, EscalationTarget::Principal(pid("psn:lead")));
    assert!(p.step_at(4).is_none(), "walk 4 is exhausted (both loops done)");
}

#[test]
fn step_at_empty_policy_is_none() {
    let p = EscalationPolicy { policy_id: "x".into(), steps: vec![], repeat: 3 };
...
fn notify_for_critical_pierces_quiet_hours_on_all_channels() {
    ...
    assert_eq!(out, chans, "critical pierces — you cannot silence an on-call page");
}
...
fn notify_for_critical_pierces_even_if_pierce_classes_omits_critical() {
    ...
    let out = notify_for(&chans, Class::Critical, &no_pierce, /* in_quiet */ true);
    assert_eq!(out, chans, "critical pages all channels regardless of pierce_classes config");
}
...
fn wheel_fire_due_is_effectively_once() {
    ...
    assert!(!w.fire_due("run-1"), "a re-fire (restart replay) is a no-op — no double page");
...
fn page_starts_the_chain_and_pages_the_first_on_call_at_fire_time() {
    ...
    let (run_id, outcome) = eng
        .page(
            tenant(), region(), "run-1".into(), policy, ArtifactRef("myelin://acme/issues/issue/7".into()),
            Some(&schedule()), 780, &never_quiet(), true,
        )
        .expect("page starts");
    assert_eq!(outcome.principal, pid("psn:bob"), "the first page reaches the on-call AT FIRE TIME");
    assert_eq!(outcome.channels, vec![Channel::InApp, Channel::WebPush], "critical pierces all channels");
...
fn empty_policy_surfaces_a_config_error_not_a_silent_drop() {
    let eng = engine();
    let policy = EscalationPolicy { policy_id: "x".into(), steps: vec![], repeat: 1 };
    let err = eng
        .page(
            tenant(), region(), "run-x".into(), policy, ArtifactRef("myelin://acme/x".into()),
            Some(&schedule()), 600, &never_quiet(), false,
        )
        .unwrap_err();
...
fn page_surfaces_no_one_on_call_when_the_schedule_does_not_cover() {
    ...
    let err = eng
        .page(
            tenant(), region(), "run-g".into(), policy, ArtifactRef("myelin://acme/x".into()),
            Some(&gap), 600, &never_quiet(), false,
        )
        .unwrap_err();
...
fn advance_walks_to_the_next_step_exactly_once() {
    ...
    let (run_id, _) = eng
        .page(
            tenant(), region(), "run-1".into(), policy, ArtifactRef("myelin://acme/x".into()),
            Some(&schedule()), 600, &never_quiet(), false,
        )
        .unwrap();
    ...
    let run = eng.run(&run_id).unwrap();
    assert_eq!(run.pages.len(), 2, "exactly two pages total — never zero, never three");
}
...
fn advance_is_effectively_once_a_replayed_fire_does_not_double_page() {
    ...
    let (run_id, _) = eng
        ...
        .unwrap();
    let _ = eng.advance(&run_id, Some(&schedule()), 600, &never_quiet(), false).unwrap();
    // A restart replays the SAME timer fire → the wheel's fire_due returns false → NO second page.
    let replay = eng.advance(&run_id, Some(&schedule()), 600, &never_quiet(), false).unwrap();
    assert_eq!(replay, None, "a replayed fire is a no-op — no double page (NOTIF-D7)");
    assert_eq!(eng.run(&run_id).unwrap().pages.len(), 2, "still exactly two pages");
}
...
fn advance_past_the_last_step_exhausts_the_chain() {
    ...
    let _ = eng.advance(&run_id, Some(&schedule()), 600, &never_quiet(), false).unwrap();
    let exhausted = eng.advance(&run_id, Some(&schedule()), 600, &never_quiet(), false).unwrap();
...
fn ack_halts_the_chain_and_emits_the_outbox_event() {
    ...
    assert!(!eng.wheel().has_timer(&run_id), "the ack cancelled the durable timer");
    // The ack EVENT rode the outbox (the ONLY emit path) — exactly one committed row.
    assert_eq!(outbox.committed_count(), 1, "notif.escalation.acked committed via the outbox");
}
...
fn ack_is_idempotent_a_double_ack_acks_once() {
    ...
    let (run_id, _) = eng
        ...
        .unwrap();
    assert!(eng.ack(&run_id, pid("psn:alice"), ts()).unwrap(), "first ack halts");
    assert!(!eng.ack(&run_id, pid("psn:bob"), ts()).unwrap(), "second ack is a no-op");
    ...
    assert_eq!(eng.run(&run_id).unwrap().acked_by, Some(pid("psn:alice")), "the first acker stands");
}
...
fn ack_after_the_window_fired_still_halts_but_a_fired_timer_does_not_page() {
    ...
    let late = eng.advance(&run_id, Some(&schedule()), 600, &never_quiet(), false).unwrap();
    assert_eq!(late, None, "no page after an ack — the chain is halted");
    assert_eq!(eng.run(&run_id).unwrap().pages.len(), 1, "still exactly one page (acked at step 0)");
}
...
fn render_oncall_shows_who_is_on_call_now_and_the_windows() {
    ...
    assert!(out.contains("now on call: psn:alice"), "shows the current on-call (opaque pseudonym)");
    assert!(out.contains("00:00–12:00) → psn:alice"), "shows the rotation windows");
...
fn render_oncall_handles_an_uncovered_window() {
    ...
    assert!(out.contains("none — uncovered window"), "surfaces no-one-on-call, never a silent gap");
}
...
fn render_page_shows_the_paged_principal_and_channels() {
    ...
    assert!(out.contains("in_app, web_push"), "shows the pierce-result channels");
...
fn chained_kill_mid_ack_window_resumes_and_pages_next_step_exactly_once() {
    ...
    let (run_id, first) = eng_a
        .page(
            tenant(), region(), "run-1".into(), policy.clone(),
            ArtifactRef("myelin://acme/issues/issue/7".into()),
            Some(&schedule()), 600, &never_quiet(), false,
        )
        .unwrap();
    assert_eq!(first.principal, pid("psn:alice"), "first page to the on-call (minute 600 → alice)");
    ...
    assert_eq!(next.principal, pid("psn:lead"), "the next step (the fixed secondary lead)");
    assert_eq!(next.walk, 1);

    // A replayed fire (a SECOND restart over the same fired timer) does NOT double-page.
    assert_eq!(
        eng_b.advance(&run_id, Some(&schedule()), 600, &never_quiet(), false).unwrap(),
        None,
        "no double page across the kill/resume — exactly once (NOTIF-D7)"
    );

    // --- the ack halts the chain ---
    assert!(eng_b.ack(&run_id, pid("psn:lead"), ts()).unwrap(), "the ack halts the chain");
