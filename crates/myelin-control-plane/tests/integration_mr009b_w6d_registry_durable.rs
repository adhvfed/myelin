...
async fn self_host_boots_on_pg_and_routing_survives_restart() {
    ...
    let pool = provider.db_pool();
    // Bridges the CounterMinter-minted tenant id (a body-local — it is NOT suffix-tagged, so the
    // general `cleanup(pool, &suffix)` LIKE-pattern sweep below cannot find it) out to the cleanup
    // closure. `with_cleanup` runs `body` to completion (or panic) and only then runs `cleanup` —
    // never concurrently — so a plain `RefCell` (no locking) is sound here.
