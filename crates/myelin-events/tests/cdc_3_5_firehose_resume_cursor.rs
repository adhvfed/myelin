...
use myelin_events::{Firehose, FirehoseError, FirehoseScope, Frame, FrameDraft, DEFAULT_INFLIGHT_CAP};
...
fn provider_publishes(fh: &mut Firehose, stream: &str, scope: &FirehoseScope, payload: &str) -> Frame {
...
fn cdc_3_5_provider_publishes_consumer_resumes_loses_zero_ops() {
    ...
    for (i, p) in ["op-1", "op-2", "op-3"].iter().enumerate() {
        ...
        assert_eq!(f.seq, (i + 1) as u64, "the transport assigns the monotonic seq, not the producer");
    }
    ...
    assert_eq!(gap, vec![3, 4, 5], "the consumer backfills (last_seq, now] — 0 lost, 0 dup");
}
...
fn cdc_3_5_out_of_window_resume_is_a_named_resync_required() {
    ...
    assert!(err.is_resync_required(), "the consumer gets a NAMED resync_required (→ *.snapshot, EB-22)");
}
...
fn cdc_3_5_consumer_over_broad_scope_is_rejected() {
    ...
    assert!(err.is_over_broad_scope(), "scope = * is rejected (the protocol's bounded-scope invariant)");
...
fn cdc_3_5_per_stream_scope_seq_is_independent() {
    ...
    assert_eq!(provider_publishes(&mut fh, "issues", &b, "y").seq, 1, "b has its own seq");
    assert_eq!(provider_publishes(&mut fh, "issues", &a, "z").seq, 2, "a's seq is independent of b");
}
