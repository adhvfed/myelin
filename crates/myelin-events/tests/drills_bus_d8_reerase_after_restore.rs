...
use myelin_events::{
    ...
    EventDraft, EventType, IdMinter, InMemoryShredder, InlinePiiShredder, InProcessBus,
...
fn bus_post_restore_re_erasure_zero_resurrected_keys_nothing_lost() {
    ...
    holder
        .erase_and_record("u42", &mut live_log, &mut live_outbox, minter(), &ledger, now())
        .expect("live erase + ledger record");

    let key = PiiKeyRef("kms://acme/0/subject:u42".into());
    assert!(!shredder.is_live(&key), "precondition: the DEK is dead in the live cell");
    assert!(ledger.is_erased("u42"), "the PII-free ledger remembers the erasure");
    ...
    shredder.seal(&key);
    assert!(shredder.is_live(&key), "the restore RESURRECTED u42's inline-PII DEK");
    ...
    assert_eq!(receipt.resurrected, 0, "STOR-D1/D2: 0 resurrected inline-PII keys post-restore");
    assert!(receipt.is_green(), "the Bus's restore-verify leg is GREEN");
    assert_eq!(receipt.keys_resurrected_by_restore, 1, "the restore brought the key back (honest)");
    ...
    assert!(restored_log.is_tombstoned("01J-1"), "the restored row carries a tombstone again");
    ...
    assert!(drain.published >= 1, "the relay published the re-emitted *.erased tombstone");
    ...
    assert!(depth_ok.is_green(), "outbox drained after re-erasing across the restore: {depth_ok:?}");
    assert!(dlq_ok.is_green(), "no tombstone dead-lettered re-erasing: {dlq_ok:?}");
}
