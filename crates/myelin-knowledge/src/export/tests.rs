...
fn para(md: &str) -> Block {
    Block::Paragraph { inline: parse_inline(md, &[]) }
}
...
fn rich_doc() -> ExportDoc {
    ...
    ExportDoc::new(
        ...
        vec![
            ...
            ExportBlock::leaf("b2", para("Plain text with `code` and *italic* and a \u{FFFC} ref")),
            ...
            ExportBlock::leaf(
                ...
                Block::BulletList {
                    items: vec![
                        ListItem { blocks: vec![para("first item")] },
                        ListItem { blocks: vec![para("second **bold** item")] },
                    ],
                },
            ),
            ExportBlock::leaf(
                ...
                Block::TaskList {
                    items: vec![
                        TaskItem { checked: true, inline: parse_inline("done", &[]) },
                        TaskItem { checked: false, inline: parse_inline("todo", &[]) },
                    ],
                },
            ),
            ExportBlock::leaf(
                "b6",
                Block::CodeBlock { lang: Some("rust".into()), text: "let x = **not bold**;".into() },
            ),
            ExportBlock::leaf(
                "b7",
                Block::Callout {
            ...
            ExportBlock::leaf(
                ...
                Block::Table {
                    columns: vec![
                        Column { header: parse_inline("Name", &[]) },
                        Column { header: parse_inline("Role", &[]) },
                    ],
                    rows: vec![vec![
                        Cell { blocks: vec![para("Alice")] },
                        Cell { blocks: vec![para("SRE")] },
                    ]],
                },
            ),
            ...
            ExportBlock::leaf(
                "b10",
                Block::Paragraph { inline: parse_inline("see \u{FFFC} here", &nodes) },
            ),
        ],
    )
}
...
fn lossless_json_roundtrips_byte_faithful() {
    ...
    assert_eq!(back, doc, "the JSON bundle is a byte-faithful lossless round-trip (Art. 20)");
}
...
fn json_roundtrip_gate_is_green() {
    ...
    let doc = rich_doc();
    assert!(doc.json_roundtrips(), "the export/import round-trip gate is green (KN-D2 + Art. 20)");
}
...
fn structured_nodes_survive_the_json_roundtrip() {
    ...
    let b10 = back.all_blocks().into_iter().find(|b| b.id.as_str() == "b10").unwrap();
    match &b10.block {
...
fn code_block_text_is_verbatim_across_roundtrip() {
    let doc = rich_doc();
    let back = ExportDoc::from_json_bundle(&doc.to_json_bundle()).unwrap();
    let b6 = back.all_blocks().into_iter().find(|b| b.id.as_str() == "b6").unwrap();
    match &b6.block {
...
fn markdown_exporter_renders_blocks() {
    ...
    assert!(md.contains("## **Incident** response"), "heading renders: {md}");
...
fn html_exporter_is_a_self_contained_document() {
    ...
    assert!(html.contains("<h2><strong>Incident</strong> response</h2>"), "marks → elements: {html}");
    ...
    assert!(html.contains("<input type=\"checkbox\" disabled checked>"), "checked task: {html}");
    assert!(html.contains("<pre><code class=\"language-rust\">"), "code block lang class");
    // code content is HTML-escaped (the `*` survives literally, no element injection)
    assert!(html.contains("let x = **not bold**;"), "code escaped verbatim: {html}");
...
fn html_escapes_dangerous_content() {
    let doc = ExportDoc::new("p", "Title <script>", None, vec![ExportBlock::leaf(
        "x",
        para("a < b & c > d"),
    )]);
    let html = doc.to_html();
    assert!(!html.contains("<script>"), "the title is escaped, no injection");
    assert!(html.contains("&lt;script&gt;"));
    assert!(html.contains("a &lt; b &amp; c &gt; d"), "body escaped: {html}");
}
...
fn pdf_exporter_emits_a_valid_pdf() {
    ...
    assert!(tail.windows(5).any(|w| w == b"%%EOF"), "the PDF trailer/EOF");
    ...
    assert!(s.contains("My Runbook"), "the title is in the content stream");
}

#[test]
fn pdf_escapes_parens_in_text() {
    let doc = ExportDoc::new("p", "f(x) and (paren)", None, vec![ExportBlock::leaf(
        "x",
        para("a (b) c"),
    )]);
    let pdf = doc.to_pdf();
    let s = String::from_utf8_lossy(&pdf);
    assert!(s.contains("f\\(x\\)"), "parens escaped in the title literal: missing in {s:?}");
}
...
fn csv_exporter_renders_header_and_rows_in_schema_order() {
    let csv = export_rows_to_csv(&schema(), &[row("r1", "Alice", 3, true), row("r2", "Bob", 0, false)]);
...
fn csv_quotes_special_characters_rfc4180() {
    let mut props: BTreeMap<FieldId, FieldValue> = BTreeMap::new();
    props.insert(FieldId::new("name"), FieldValue::Text("Smith, \"Bob\"".into()));
    ...
    assert!(csv.contains("\"Smith, \"\"Bob\"\"\""), "comma+quote escaped per RFC-4180: {csv}");
}
...
fn csv_absent_field_is_empty_cell() {
    ...
    assert_eq!(data, "Only,,", "absent fields are empty cells, schema order preserved");
}
...
fn adf_import_lossless_document_records_nothing() {
    ...
    assert!(result.report.is_lossless(), "a direct-equivalent import records no loss");
    ...
    assert!(matches!(result.doc.blocks[1].block, Block::Paragraph { .. }));
...
fn adf_import_records_each_lossy_conversion() {
    ...
    let result = import_adf("p1", "Imported", &nodes);
    assert_eq!(result.report.loss_count(), 3, "three lossy nodes recorded, the lossless one not");
    assert_eq!(result.report.conversions[0].node, AdfNode::Mention);
    assert_eq!(result.report.conversions[0].degraded_to, AdfTarget::PlainText);
    ...
    match &result.doc.blocks[2].block {
        Block::Callout { tone, blocks } => {
            ...
            let md = blocks
                ...
                .collect::<Vec<_>>()
                .join(" ");
            assert!(md.contains("unsupported macro: jira-macro"), "the marker is in-content: {md}");
        }
...
fn adf_import_conditional_node_is_lossless_when_resolved() {
    ...
    let resolved = import_adf("p", "t", &[ParsedAdfNode::resolved(AdfNode::Mention, "alice")]);
    assert!(resolved.report.is_lossless(), "an in-tenant mention is lossless");
    match &resolved.doc.blocks[0].block {
        Block::Paragraph { inline } => {
            assert_eq!(inline.nodes.len(), 1, "the structured mention node survives");
...
fn adf_import_degraded_content_survives_not_dropped() {
    ...
    let r = import_adf("p", "t", &[ParsedAdfNode::unresolved(AdfNode::Mention, "ext-user")]);
    assert_eq!(r.report.loss_count(), 1);
    match &r.doc.blocks[0].block {
        Block::Paragraph { inline } => {
            let txt = myelin_content::serialize_inline(inline);
            assert!(txt.contains("ext-user"), "the degraded text survives: {txt}");
...
fn adf_imported_doc_json_roundtrips() {
    ...
    assert!(result.doc.json_roundtrips(), "the imported doc round-trips losslessly as a bundle");
}
