...
use myelin_knowledge::{
    ...
    PropertyBag, RelationKind, RelationStore, RollupFn, RollupResolver, MAX_FORMULA_DEPTH,
    MAX_FORMULA_NODES,
};
...
fn cdc_13_3_formula_value_space_is_the_frozen_literal() {
    ...
    .unwrap();
    let out = compute_row(&viewer("p:v"), "src", &FieldId::new("f"), &BTreeMap::new(), &schema, &r);
    assert_eq!(out, CellValue::Int(42), "the formula evaluates over the frozen Literal value space");
}
...
fn cdc_13_3_cycle_gate_surfaces_cycle_never_loops() {
    ...
    let schema = FormulaSchema::of([
        FormulaField { field: FieldId::new("a"), expr: FormulaExpr::FormulaRef(FieldId::new("b")) },
        FormulaField { field: FieldId::new("b"), expr: FormulaExpr::FormulaRef(FieldId::new("a")) },
    ])
    .unwrap();
    let out = compute_row(&viewer("p:v"), "src", &FieldId::new("a"), &BTreeMap::new(), &schema, &r);
    assert_eq!(out, CellValue::Cycle, "a cyclic formula is #CYCLE (bounded — the eval terminated)");
    assert_eq!(out.display(), "#CYCLE", "the diagnostic cell renders #CYCLE");
}
...
fn cdc_4_3_rollup_permission_gate_zero_leak() {
    ...
    authz.grant(&tenant, &region, &v.principal_id.0, "read", "t:1", "zk-0000000001");
    authz.grant(&tenant, &region, &v.principal_id.0, "read", "t:2", "zk-0000000001");
    ...
    let sum = compute_row(&v, "src", &FieldId::new("total"), &BTreeMap::new(), &sum_schema, &r);
    let count = compute_row(&v, "src", &FieldId::new("n"), &BTreeMap::new(), &count_schema, &r);
    assert_eq!(sum, CellValue::Int(30), "0 rollup leak: SUM = 30 (10+20), NOT 100030 — the restricted target is unsummed");
    assert_eq!(count, CellValue::Int(2), "0 rollup leak: COUNT = 2, NOT 3 — the restricted target is uncounted");
    ...
    authz.grant(&tenant, &region, &v2.principal_id.0, "read", "t:1", "zk-0000000002");
    authz.grant(&tenant, &region, &v2.principal_id.0, "read", "t:2", "zk-0000000002");
    authz.grant(&tenant, &region, &v2.principal_id.0, "read", "t:secret", "zk-0000000002");
    let sum2 = compute_row(&v2, "src", &FieldId::new("total"), &BTreeMap::new(), &sum_schema, &r);
    assert_eq!(sum2, CellValue::Int(100_030), "the authorized viewer sees the full SUM (per-viewer conjoin, not a blanket hide)");
}
