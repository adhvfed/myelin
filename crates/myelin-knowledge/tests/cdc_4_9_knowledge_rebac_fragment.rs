...
fn cdc_4_9_read_names_agree_with_the_write_carrier_and_the_frozen_vocabulary() {
    ...
    assert_eq!(object_types::BLOCK, myelin_content::rebac_fragment::object_types::BLOCK);
    assert_eq!(
    ...
    for r in [PARENT_PAGE, PARENT_SPACE, DIRECT_READER, DIRECT_BLOCK, WATCHER] {
        assert!(page.relations.iter().any(|rel| rel.0 == r), "page declares relation `{r}`");
    }
    for p in [READ, "comment", EDIT] {
        assert!(page.permissions.iter().any(|perm| perm.0 == p), "page declares permission `{p}`");
    }

    // the space + block + database_row vocabularies (§5).
    assert!(space_read_fragment().permissions.iter().any(|p| p.0 == READ));
    assert!(block_read_fragment().relations.iter().any(|r| r.0 == PARENT_PAGE));
    let row = database_row_read_fragment();
    assert!(row.relations.iter().any(|r| r.0 == ROW_READER), "database_row declares row_reader");
    assert!(row.permissions.iter().any(|p| p.0 == VIEW_FIELD), "database_row declares view_field");
}
...
fn cdc_4_9_page_read_override_formula_is_correct() {
    ...
    assert!(!blocked.contains("alice"), "the - direct_block override removes the inheriting alice");
    assert!(blocked.contains("bob"), "an un-blocked inheriting reader stays");

    // direct_reader ADDS a sub-page (the + direct_reader arm).
    let added = page_read_override(&["alice"], &["carol"], &[]);
    assert!(added.contains("carol"), "the + direct_reader arm adds carol");
    ...
    assert_eq!(plain.len(), 2, "with no override, read is exactly the inherited set");
}
