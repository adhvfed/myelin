...
async fn dedicated_scheduler_role_is_region_bound_least_privilege_and_reset_safe() {
    ...
    common::with_privilege_fixture_lock(
        ...
        || async {
            ...
            let admin_url = configured_url("DATABASE_MIGRATION_URL", ADMIN_DEFAULT);
            let scheduler_url =
                configured_url("MYELIN_CI_SCHEDULER_DATABASE_URL", SCHEDULER_DEFAULT);
            let admin = PgPoolOptions::new()
                .max_connections(2)
                .connect(&admin_url)
                .await
        ...
        "the guard observes this fixture in addition to any historical dev-stack backlog"
          ...
          WHERE tenant_id = ANY($1) AND state = 'leased'",
                ...
                let reaper =
                    JobQueueReaper::new(region_store.clone(), FR_PAR, Duration::from_secs(15));
                assert!(
        ...
        "the production reaper capability recovers expired leases across same-region tenants"
          ...
          WHERE tenant_id = ANY($1) AND state = 'queued' AND lease_owner IS NULL",
            ...
            .expect("set session residue");
                ...
                let escaped_updates =
                    sqlx::query("UPDATE job_queue SET state = 'queued' WHERE tenant_id = $1")
                        .bind(&tenant_a)
                        .execute(&mut *tenant_escape)
                        .await
                ...
                let other_region_runs: Vec<String> =
                    sqlx::query_scalar("SELECT tenant_id FROM ci_run")
    ...
    .expect("inspect app membership");
                ...
                .expect("raw DSNs differ for identity probe");
                let admin_refusal =
                    match CiSchedulerDbProvider::connect(admin_as_scheduler, &app).await {
                        Err(error) => error,
                        Ok(_) => panic!("admin credential must be rejected"),
                    };
                assert_eq!(admin_refusal, CiSchedulerDbError::Superuser);
                let wrong_region_config = CiSchedulerDbConfig::from_parts(
                    scheduler_url,
                    &app_url,
                    &admin_url,
                    DE_FRA.to_owned(),
                ...
                let region_refusal =
                    match CiSchedulerDbProvider::connect(wrong_region_config, &app).await {
                        Err(error) => error,
