...
use myelin_gdpr_service::{
    ...
    AUDIT_CARVE_OUT_STORE, EraseChecklist, GitDbHolder, KnowledgeAgentTraceHolder,
    KnowledgeStoreHolder, KnowledgeStoreModel, ProducerHolderRegistration, ShredKeyClass,
...
fn producer_holder_erasure_gate_kn_d4_and_kn_d12_are_green() {
    ...
    assert!(inv.holders.contains("oltp:git_oltp"), "H1 Git is in the data map");
    assert!(inv.holders.contains("oltp:knowledge_oltp"), "H4 Knowledge is in the data map");
    assert!(inv.holders.contains("oltp:agent_fabric_trace"), "H17 agent-trace is in the data map");
    assert!(
    ...
    assert!(kms.is_present(&subject_dek("u-subject")), "the per-subject DEK is live before erase");
    assert_eq!(knowledge.reidentify_hits("u-subject"), 1, "the embedding re-identifies before erase");
    assert!(trace.has_trace("u-subject"), "the run trace is present before erase");
    ...
    let receipts = orch.fan_out_erase(&subject_scope("u-subject"), &checklist).unwrap();
    ...
    for r in &receipts {
        ...
        assert!(r.receipt.receipt.content_hash.starts_with("blake3:"), "each receipt is content-addressed");
    }

    // ─── Step 3: KN-D4 — 0 recoverable incl. vectors ───
    assert!(!kms.is_present(&subject_dek("u-subject")), "the per-subject DEK is destroyed (free-text unrecoverable)");
    assert_eq!(
    ...
    assert!(!trace.has_trace("u-subject"), "KN-D12: the agent trace is crypto-shredded (0 recoverable)");
    assert_ne!(
        producer_holder_ids::AGENT_TRACE, AUDIT_CARVE_OUT_STORE,
...
fn producer_fan_out_is_resumable_no_double_shred() {
    ...
    let kn_h = KnowledgeStoreHolder::new(&knowledge, &kms);
    let trace_h = KnowledgeAgentTraceHolder::new(&trace, &kms);
    let orch = UpstreamHolderOrchestrator::new(ProducerHolderRegistration::register_producers(vec![
        (producer_holder_ids::GIT_DB, &git_h as &dyn PersonalDataHolder),
        (producer_holder_ids::KNOWLEDGE_DB, &kn_h),
        (producer_holder_ids::AGENT_TRACE, &trace_h),
    ]));
    ...
    let first = orch.fan_out_erase(&subject_scope("u-resume"), &checklist).unwrap();
    ...
    let second = orch.fan_out_erase(&subject_scope("u-resume"), &checklist).unwrap();
    assert_eq!(first, second, "an idempotent re-drive returns the SAME receipts (0 double-erase)");
    assert_eq!(trace.erase_call_count(), 1, "the trace was shredded exactly once across the re-drive");
    assert_eq!(knowledge.erase_call_count(), 1, "the knowledge embedding was purged exactly once");
}
