...
fn gate_ediscovery_export_is_inclusion_proof_bearing_and_legal_hold_frozen() {
    ...
    authority.consumer().handle(&action("1", "acme", "u-A"));
    authority.consumer().handle(&action("2", "acme", "u-B"));
    authority.consumer().handle(&action("3", "acme", "u-A"));
    ...
    let bundle = exporter.export(&scope, "2026-06-20T01:00:00Z").expect("export");

    // INCLUSION PROOF: every record verifies against the bundle STH (the Merkle tree root).
    assert!(bundle.verify(authority.key()), "the bundle verifies end-to-end");
    for r in &bundle.records {
        assert!(verify_inclusion(&r.inclusion, &bundle.sth), "record {} is Merkle-proven", r.seq);
    }
    ...
    let principal = Principal::stub(PrincipalId("u-A".into()), PrincipalKind::Human, TenantId("acme".into()));
    let erase = EraseScope::Subject { subject: SubjectRef::new(principal), tenant: TenantId("acme".into()) };
    assert_eq!(holds.verdict(DsrKind::Erasure, &erase), HoldVerdict::Deferred, "the scope is frozen");
}
...
fn gate_history_rewrite_skeleton_is_a_resumable_idempotent_activity() {
    ...
    assert_eq!(first.action, "git.history_rewrite", "audited as git.history_rewrite");
    ...
    assert_eq!(activity.phase_call_count(RewritePhase::Audit), 1, "phase 0 survived");
    assert_eq!(activity.phase_call_count(RewritePhase::CryptoShredPackTier), 2, "phase 2 re-ran once");
    ...
    assert!(first.residual_named.contains("P-GA-35"), "the residual names its follow-on");
}
