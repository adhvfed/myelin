...
impl RankedList {
    ...
    pub fn from_ranked(doc_ids: impl IntoIterator<Item = impl Into<String>>) -> RankedList {
        RankedList { doc_ids: doc_ids.into_iter().map(Into::into).collect() }
    }
...
pub fn fuse_with_k(branches: &[RankedList], k: f32) -> Vec<FusedHit> {
    ...
    let mut fused: Vec<FusedHit> =
        scores.into_iter().map(|(doc_id, score)| FusedHit { doc_id: doc_id.to_string(), score }).collect();
...
mod tests {
    ...
    fn a_doc_ranked_by_both_branches_outranks_a_one_branch_doc() {
        ...
        assert_eq!(fused[0].doc_id, "a", "the doc both branches rank surfaces first (the RRF boost)");
        ...
        assert!(a > b, "two-branch agreement ({a}) beats single-branch top ({b})");
    }
    ...
    fn fusion_is_score_scale_free_depends_only_on_rank() {
        ...
        assert_eq!(ids(&a), ids(&b), "fusion depends only on rank — reproducible, scale-free");
    }
    ...
    fn rrf_never_introduces_a_doc_absent_from_every_branch() {
        ...
        assert_eq!(got, union, "the fused set is EXACTLY the union of the branch lists — no new doc");
        ...
        assert!(!got.contains("secret"), "a doc in no branch never appears (no fusion-introduced leak)");
    }
    ...
    fn fusion_is_deterministic_with_doc_id_tiebreak() {
        ...
        assert_eq!(ids(&fused), vec!["k", "m"], "tie broken by doc_id ascending");
        // Swapping the branch order does not change the deterministic output.
        let swapped = reciprocal_rank_fusion(&[vec, ft]);
        assert_eq!(ids(&swapped), vec!["k", "m"], "stable regardless of branch order");
    }
    ...
    fn weighting_is_additive_one_based_k_plus_rank() {
        ...
        assert_eq!(fused[0].doc_id, "z", "additive k+rank: two rank-2 hits beat one rank-1 hit");
        let z = fused.iter().find(|h| h.doc_id == "z").unwrap().score;
        let a = fused.iter().find(|h| h.doc_id == "a").unwrap().score;
        assert!(z > a, "the two-branch rank-2 doc strictly out-scores the one-branch rank-1 doc ({z} > {a})");
        // Pin the exact value so a 0-based (`rank0` instead of `rank0+1`) mutant is caught: `a` is
        // rank 1 ⇒ 1/(60+1) = 1/61.
        let expected_a = 1.0 / (RRF_K + 1.0);
        assert!((a - expected_a).abs() < 1e-6, "rank-1 contribution is exactly 1/(k+1) = {expected_a}");
    }
    ...
    fn empty_branch_is_a_no_op() {
        ...
        assert_eq!(ids(&fused), vec!["a", "b", "c"], "the non-empty branch's order is preserved");
    ...
    fn earlier_rank_contributes_more() {
        ...
        assert_eq!(ids(&fused), vec!["first", "second", "third"], "rank order preserved by RRF");
        assert!(
