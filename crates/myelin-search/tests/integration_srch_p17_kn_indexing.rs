...
use myelin_content::{Block, EmbedDisplay, HeadingLevel, InlineNode, parse_inline, OBJ};
...
fn fr_page_blocks() -> Vec<Block> {
    ...
    vec![
        ...
        Block::CodeBlock { lang: Some("rust".into()), text: "fn ordonnanceur_interblocage() {}".into() },
        Block::Embed { reference: embedded, display: EmbedDisplay::Card },
    ]
}
...
fn en_page_blocks() -> Vec<Block> {
    vec![
        ...
        Block::Paragraph { inline: parse_inline("the scheduler avoids deadlock at runtime", &[]) },
    ]
}
...
fn kn_pages_index_multilingual_and_are_searchable() {
    ...
    fetcher.put(fr_ref, page_search_projection(&fr_page_blocks(), Some("fr")));
    fetcher.put(en_ref, page_search_projection(&en_page_blocks(), Some("en")));
    let ix = kn_indexer(fetcher);

    // Index both pages through the live per-event pipeline (project → analyze → embed → upsert).
    ix.index(&kn_event("e-fr", "knowledge.page.created", fr_ref)).expect("index fr page");
    ix.index(&kn_event("e-en", "knowledge.page.created", en_ref)).expect("index en page");
    assert_eq!(ix.live_count(&tenant(), &region()), 2, "both KN pages are live");
    ...
    let fr_hits = ix.search_ft(&tenant(), &region(), &acl, "ordonnanceur", 10).expect("fr search");
    assert!(fr_hits.iter().any(|h| h.doc_id == fr_ref), "the FR term finds the FR page");
    assert!(!fr_hits.iter().any(|h| h.doc_id == en_ref), "the FR term does not find the EN page");

    // EN term hits the EN page.
    let en_hits = ix.search_ft(&tenant(), &region(), &acl, "deadlock", 10).expect("en search");
    assert!(en_hits.iter().any(|h| h.doc_id == en_ref), "the EN term finds the EN page");
    ...
    let q = embedder.embed("scheduler deadlock design").expect("query embeds");
    let sem = ix.search_semantic(&tenant(), &region(), &acl, &q, 5).expect("semantic search");
    assert!(!sem.is_empty(), "semantic KN search returns a visible passage (vector-in-v1, §4.5)");
    assert!(
...
fn kn_structured_inline_node_facets_filter_correctly() {
    ...
    let fetcher = Arc::new(KnFetcher::default());
    fetcher.put(fr_ref, page_search_projection(&fr_page_blocks(), Some("fr")));
    fetcher.put(plain_ref, page_search_projection(&en_page_blocks(), Some("en")));
    let ix = kn_indexer(fetcher);
    ix.index(&kn_event("e-fr", "knowledge.page.created", fr_ref)).expect("index");
    ix.index(&kn_event("e-pl", "knowledge.page.created", plain_ref)).expect("index");
    ...
    let m = ix
        .search_structured(&tenant(), &region(), &acl, FACET_MENTION, &FieldValue::Relation("alice".into()), 10)
...
fn kn_db_row_custom_field_query_via_gin_scan() {
    ...
    fa.insert("order_key".to_string(), FieldValue::OrderKey(OrderKey::bisect(None, None)));
    ...
    fb.insert("order_key".to_string(), FieldValue::OrderKey(OrderKey::bisect(None, None)));
    fetcher.put(row_a, SearchProjection { text: "ship the scheduler".into(), fields: fa, lang: None });
    fetcher.put(row_b, SearchProjection { text: "write the docs".into(), fields: fb, lang: None });
    let ix = kn_indexer(fetcher);
    ix.index(&kn_event("r-a", "knowledge.row.updated", row_a)).expect("index row a");
    ix.index(&kn_event("r-b", "knowledge.row.updated", row_b)).expect("index row b");

    let acl = AclFilter::ids([row_a, row_b]);
    // The GIN-scan facet query: priority == high → exactly row_a.
    let hits = ix
        .search_structured(&tenant(), &region(), &acl, "priority", &FieldValue::Select("high".into()), 10)
        .expect("custom-field GIN scan");
    assert_eq!(hits.len(), 1, "exactly the high-priority row (the GIN-scan custom field)");
...
fn kn_rollup_field_is_read_time_not_a_stored_indexed_value() {
    ...
    assert_eq!(schema.get("rollup_total").unwrap().kind, FieldKind::ReadTime);
    ...
    assert_eq!(plan.post_fetch.len(), 1, "the rollup Cmp lowers to a post-fetch predicate");
    ...
    assert!(stored_plan.post_fetch.is_empty(), "a stored field is not post-fetch");
    assert_eq!(stored_plan.structured.len(), 1, "a stored custom field IS a structured clause");
}
...
fn srch_d1_private_kn_page_never_leaks() {
    ...
    let ix = kn_indexer(fetcher);
    ix.index(&kn_event("v", "knowledge.page.created", visible)).expect("index visible");
    ix.index(&kn_event("p", "knowledge.page.created", private)).expect("index private");
    assert_eq!(ix.live_count(&tenant(), &region()), 2, "both pages are indexed");
    ...
    let hits = ix.search_ft(&tenant(), &region(), &acl_unauth, "zarquon", 10).expect("ft");
    assert_eq!(hits.len(), 1, "0 count-leak: exactly the one visible page (hidden page never counted)");
    assert_eq!(hits[0].doc_id, visible);
    assert!(!hits.iter().any(|h| h.doc_id == private), "0 leak: the private page never surfaces");

    // Semantic: even if the private page is the STRONGER semantic match, it never surfaces.
    let embedder = MockEmbeddingAdapter::new(16);
    let q = embedder.embed("classified zarquon deadlock plan").expect("embeds");
    let sem = ix.search_semantic(&tenant(), &region(), &acl_unauth, &q, 5).expect("semantic");
    assert!(
    ...
    let granted = ix.search_ft(&tenant(), &region(), &acl_granted, "zarquon", 10).expect("ft granted");
    assert_eq!(granted.len(), 2, "after the grant BOTH pages surface (the rejection was the ACL, not a deny)");
    assert!(granted.iter().any(|h| h.doc_id == private), "the granted page now appears");
}
...
fn srch_d3_cross_tenant_kn_pages_do_not_leak() {
    ...
    let fetcher = Arc::new(KnFetcher::default());
    fetcher.put(acme_page, page_search_projection(&en_page_blocks(), Some("en")));
    fetcher.put(evil_page, page_search_projection(&en_page_blocks(), Some("en")));
    let ix = kn_indexer(fetcher);
    ix.index(&event_in("a", "knowledge.page.created", acme_page, "acme")).expect("index acme");
    ix.index(&event_in("e", "knowledge.page.created", evil_page, "evil")).expect("index evil");
    ...
    let acme_hits = ix
        .search_ft(&acme_t, &region(), &AclFilter::ids([acme_page]), "scheduler", 10)
        .expect("acme search");
    assert!(acme_hits.iter().any(|h| h.doc_id == acme_page), "acme sees its own page");
    ...
    let cross = ix
        .search_ft(&acme_t, &region(), &AclFilter::ids([evil_page]), "scheduler", 10)
        .expect("cross-tenant search");
    assert!(cross.is_empty(), "0 cross-tenant: acme's index holds none of evil's pages");

    // And the evil tenant's index, conversely, holds only evil's page.
    let evil_hits = ix
        .search_ft(&evil_t, &region(), &AclFilter::ids([acme_page]), "scheduler", 10)
        .expect("evil search");
    assert!(evil_hits.is_empty(), "0 cross-tenant: evil's index holds none of acme's pages");
}
