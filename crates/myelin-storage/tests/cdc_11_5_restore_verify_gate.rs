...
fn ci_gate_caller_gets_a_green_artifact_on_a_whole_restore() {
    ...
    source.append(90, "r1");
    let rows = vec![WalRow { id: "r1".into(), written_at: 90, blob_ref: Some(objects[0].content_address.clone()) }];
...
fn ci_gate_caller_fails_ci_on_a_corrupted_backup() {
    ...
    let rows = vec![WalRow { id: "r1".into(), written_at: 50, blob_ref: Some(missing) }];
    let ledger = ErasureLedger::new();
    let inputs = GateInputs {
        archiver: &arch, target: 100, rows: &rows, objects: &objects,
        source: &source, kms: &kms, erasure_ledger: &ledger,
