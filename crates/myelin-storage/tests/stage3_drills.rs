...
async fn drill1_outbox_no_loss_under_crash() {
    ...
    // Connected HERE (ahead of the wrapped body below, moved up from its original position right
    // before step (2)) so `bus.purge()` is available to the cleanup closure regardless of how the
    // body exits — the cleanup closure cannot reach a `bus` that is only constructed inside the
    // body it wraps.
    ...
    common::with_cleanup(
        || async {
            ...
            //     domain state change (BUS-D4 emit-iff-committed). A committed state change ALWAYS has
            //     its outbox row; a rolled-back tx writes neither (no ghost).
            ...
            //     broker but DROP the transaction before recording `published_at` for ANY of them.
            //     Those 3 rows are now published-to-NATS yet still `published_at IS NULL` in PG — the
            //     exact silent-loss window (the relay forwarded the event then died before recording
            //     it did).
            ...
            // (3) RESTART the relay: re-claim the unsent rows and re-publish. The 3 crash-window rows
            //     are re-published; the broker dedups them on Nats-Msg-Id = event_id (0 ghost). The
            //     remaining rows are published fresh. After this pass the outbox fully drains.
            ...
            //     event_id delivered, none lost, none ghosted (the broker dedup collapsed the
            //     re-publish).
            ...
            // No event without its committed state change (emit-iff-committed): every delivered
            // event_id has a row in the co-committed state table.
        ...
        || async {
            // See `common::delete_outbox_for_aggregate` for why this isn't a bare
...
async fn drill2_restore_verify_cross_seam() {
    ...
    // The consistent point T=3. Hoisted above the wrapped body (below) so the cleanup closure can
    // deterministically re-derive the same blob bytes/hashes the body writes, without needing to
    // read the body's own locals (this crate's `common::with_cleanup` takes two independent
    // closures — see tests/common/mod.rs).
    const T: i64 = 3;

    // Wrapped so a mid-test assertion failure or panic still drops this run's tagged
    // tables + blobs, instead of only the happy path reaching the final cleanup below (see
    // tests/common/mod.rs).
    common::with_cleanup(
        || async {
            // (1) Write rows + blobs + advance the bus offset together, up to the consistent point T.
            ...
            // CAPTURE the consistent point T: the snapshot is (rows seq<=T, the blob set they
            // reference, the bus offset == T). This is the one cross-seam cursor restore lands every
            // tier at.
            ...
            // (2) MUTATE PAST T: a new row at seq=T+1 referencing a fresh blob, and the bus advanced
            //     past T. This is the divergence a crash/restore must roll back.
            ...
            // (3) RESTORE PG to T: PITR drops every row whose seq > T (the §7.3 restore_to_offset
            //     shape), and lands the bus offset back at T (the cursor cannot point past the
            //     restored rows).
            ...
            // (4a) rows <-> blob refs: every restored row's referenced blob is PRESENT in RustFS AND
            //      its bytes re-hash to the stored address (no dangling ref, no silent corruption).
            ...
            for (seq, blob_hex) in &restored_rows {
                ...
                // PRESENCE + checksum parity: get() re-hashes on read and refuses a corrupt object,
                // so a successful get IS the no-loss / checksum-parity proof for this referenced
                // blob.
            ...
            // (4b) rows <-> bus offset: the restored bus offset is exactly the max restored row seq
            //      — no offset past the restored rows (the post-T event is not referenced by any
            //      cursor).
            ...
            // (4c) the POST-T mutation is GONE: its row is absent (so it cannot dangle a blob ref
            //      through the row table) and the bus offset no longer references its event. (The
            //      post-T blob may still sit in the object store, but with NO row pointing at it —
            //      the safe direction; the forbidden direction is a row -> missing blob, which 4a
        ...
        || async {
            // Blob hashes are BLAKE3 of deterministic bytes derived only from `tag`/`seq` (never
            // read back from the body's `blob_at`/`post_hash` locals — the two closures here are
            // independent, so cleanup re-derives instead of sharing body-scoped mutable state).
            for seq in 1..=T {
                let bytes = format!("drill2 blob payload seq={seq} tag={tag}").into_bytes();
...
async fn drill3_tenant_region_rls_isolation() {
    ...
    // Wrapped so a mid-test assertion failure or panic still drops both tenants' seeded rows,
    // instead of only the happy path reaching the final cleanup below (see tests/common/mod.rs).
    common::with_cleanup(
        || async {
            // Seed tenant A's rows and tenant B's rows into the SAME table. put_tuple sets the
            // session (tenant, region) scope before each insert, so each row is written in its own
            // tenant's partition (and the WITH CHECK half of the RLS policy admits it).
            admin
            ...
            // load-bearing part: a superuser / BYPASSRLS role would silently ignore the policy.
            // myelin_app does NOT, so RLS is actually enforced for this connection.
            ...
            // (3) The app-layer reverse_index (the real authz access path) is ALSO RLS-scoped:
            //     querying as tenant A for a subject that only exists in tenant B returns ZERO rows
            //     — there is no app code path that reaches across the tenant boundary (the verified
            //     tenant is threaded AND RLS is in force; defence in depth).
            let cross = app
...
async fn drill4_rebac_check_list_objects_no_leak_no_n_plus_1() {
    ...
    // Wrapped so a mid-test assertion failure or panic still drops this tenant's seeded rows,
    // instead of only the happy path reaching the final cleanup below (see tests/common/mod.rs).
    common::with_cleanup(
        || async {
            // Load REAL tuples: alice is reader on a set of objects; bob + mallory are reader on a
            // DISJOINT set (the objects alice must NOT see — the leak candidates). A larger object
            // population for alice makes the no-N+1 assertion meaningful (one query, not
            // one-per-object).
            ...
            // (2) list_objects NO-LEAK: alice's visible set is EXACTLY her objects — bob's are not
            //     present.
            let visible = store
            ...
            // and the cross-check: every returned object genuinely passes check() (the list is
            // sound — it does not over-return).
            for o in &visible {
                assert!(
                    store
            ...
            // (3) NO N+1: list_objects over the many-object tenant is EXACTLY ONE authz query — not
            //     one check() per candidate. Snapshot the counter, run list_objects, assert delta ==
            //     1.
