...
impl<'a> RestoreVerifyJob<'a> {
    ...
    fn boot(kms: &'a KmsEngine, tail: u64) -> Self {
        ...
        archiver
            .archive_segment(WalSegment { end_offset: 0, committed_at: 0 })
            .unwrap();
        archiver.take_base_backup(1);
        archiver
            .archive_segment(WalSegment { end_offset: tail, committed_at: 10 })
            .unwrap();
...
fn restore_caller_lands_a_consistent_point() {
    ...
    let report = job.restore(100, &rows, &blobs, &source).expect("a consistent restore passes");

    // The report shape the consumer (P-061's gate) depends on:
    assert_eq!(report.restored_to_offset, 100, "restored to the cross-seam point T");
    assert_eq!(report.oltp_rows.len(), 2, "OLTP rows ≤ T (the row past T was dropped)");
    assert_eq!(report.dangling_ref_count, 0, "no-loss: 0 dangling blob refs");
    assert!(report.derived.has_doc("r1") && report.derived.has_doc("r2"), "derived == source-replay");
...
fn restore_caller_fails_on_a_missing_referenced_blob() {
    ...
    let rows = vec![
        ...
        WalRow { id: "bad".into(), written_at: 90, blob_ref: Some(h("gone")) },
    ];
...
fn restore_caller_does_not_resurrect_a_shredded_tenant() {
    ...
    assert!(report.restored_key_for_tenant(&live), "the live tenant's KEK is restored");
    assert!(
