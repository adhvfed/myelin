...
use myelin_storage::{
    ArchiveError, FirehoseArchiver, KekId, KmsEngine, SealedSegment,
};
...
impl OpStreamArchiver {
    ...
    fn boot(tenant: TenantId, region: Region, engine: Arc<KmsEngine>, stream: &str, scope: &str) -> OpStreamArchiver {
        OpStreamArchiver {
...
fn cdc_11_8_non_ci_producer_seals_and_resolves_through_the_seam() {
    ...
    for i in 1..=6u64 {
        ...
        assert_eq!(seq, i, "the 3.5 transport assigns a monotonic per-(stream,scope) seq");
    }
    ...
    assert_eq!((segment.first_seq, segment.last_seq, segment.frame_count), (2, 5, 4));
    let pointer = segment.pointer_payload();
    assert!(pointer.contains(&segment.content_hash.to_multihash_string()));
    assert!(!pointer.contains("op-2"), "11.8: the durable pointer must NOT inline the frame body");
    ...
    assert_eq!(resolved, live, "11.8: the archived segment matches the live firehose tail");
