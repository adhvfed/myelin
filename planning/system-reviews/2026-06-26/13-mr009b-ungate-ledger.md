...
| **W4** | ~~Control-plane re-point~~ **RECLASSIFIED → build-then-wire (W6-kind), NOT a re-point.** Grounding found: `Registry` is the pervasive always-compiled production API of the whole CP crate (impls across discover/place/placement_of/multi_cell; a field of `DegenerateControlPlane` self-host, which uses the in-memory Registry+MisrouteAudit as its REAL system-of-record in production). The durable backing (`DurablePlacementBacking`) covers only cell+tenant_placement+audit; `Registry`'s load-bearing surface — `repo_placements` (real repo→cell routing, NOT rebuildable, NO table), provisioning log, local_tenants, status setters, discover — has NO durable backing. Honest flip = BUILD those durable backings + migrations (repo_placement table etc.) + convert `Registry` to a role-struct over `PlacementBackend{Memory[test-support], Pg}` covering its WHOLE surface + re-point self-host/boot. Moved into the W6 build cluster (as **W6d**). | 2 | (deferred to W6d) | build + wire |
| **W5** | **KMS durable-default (SI-006) — DONE (`c271932`, 2026-07-15), 12→11.** `KmsCore` + `KmsBackend{Memory[test-support], Durable(DurableKms)}`; `load_or_generate` = production constructor; write-through fail-static (mint persists before handout; shred deletes durable row first). Site audit: 1 prod root (edge main.rs, fail-loud), 110 cfg(test), 7 drill fns gated, 0 injection re-points. Verifier CONFIRMED-SOUND; D1 (rotate persist non-transactional) fixed pre-commit + regression test; residuals (D2 mint-visibility race, Result-conversion ripple, rotate fault-injection) in ledger 14. | 1 | 12→11 | reshape (careful) |
