...
| **W3** | **Events pipeline durable-default** — DedupLedger (SI-023) default `durable`; OutboxStore (SI-007) re-point emit callers to `PgRelay::co_commit_in_tx`, gate the in-memory Outbox behind test-support. | 2 | 14→12 | wire + re-point |
