...
| R3.2 | PR diff / files-changed (head_oid vs base_ref; side-by-side+unified; comment anchoring; G-7 keyboard/SR) | ux-git crit 2 | IN PROGRESS (thread store ready) | — | — |
...
**R3-EXIT BLOCKER (open, ledger task #13):** R3.3 thread-store verifier found the review-batch
write path (submit/discard/add-pending) enforces NO batch-author identity — any Push holder can
force-submit another reviewer's private draft (verdict+identity FORGERY: published batch shows
"A approved" with attacker text) or discard it. Read-side isolation, write=Push, IDOR, self-approval,
agent-advisory, R-BATCH-1 all HELD. Fix = actor==batch.reviewer check in submit/discard/add-pending;
sequenced immediately after R3.2 (which is editing the same crate — no concurrent-edit race). Must
land + re-verify before the exit gate below.
