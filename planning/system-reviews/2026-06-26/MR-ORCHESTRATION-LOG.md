...
| GT-003 | Durable git API writes + merge-gate + crash reconciler (E1.2; wires GT-001 at the live front door) | git_durable.rs (durable backend at the front door) + pr_store.rs (durable PR/review + repo-owned branch protection) + reconcile.rs; create-repo/web-edit/PR/review/merge persist | INDEP SECURITY verifier: REJECT → found a merge-gate BYPASS (branch-protection policy author-controlled at PR-open, default-OPEN → any tenant member merges unreviewed code) → builder fixed: repo-owned admin-set ruleset (default-CLOSED for protected refs), facts only from authorized producers, no self-approval, head_oid validated (commit + descendant); 4 regression tests close it. Durable-writes/reconciler/tenant-isolation confirmed sound | GREEN (75 git+edge + security regressions + lints + workspace + git fsck) | (this) |

| GT-004 | Git web UI — browse surface + PR overview (E1.3) | apps/web Solid routes (repo home/blob/commit-log/diff/PR overview) on the MR-019 shell; 3 new tenant-scoped read endpoints (commit_log/commit_detail reuse GixCore/libgit2); 13 Playwright+axe e2e | orch verify (real browser): Playwright 13/13 (axe-clean per screen + keyboard + the blocked-merge gate REASON shown, not bypassed); CAUGHT that apps/web was never strict-typechecked (vinxi build skips tsc) → real Toast-API + undefined-param errors → builder fixed + WIRED tsc into the gate (build=tsc&&vinxi, pnpm -r typecheck all 3 pkgs) | GREEN (pnpm build/lint/test/typecheck + Playwright 13/13 + Rust git+edge 75/0 + workspace) | (this) |

**Frontend gate note (carried forward):** the IDE/harness TS-server reports false JSX/type errors for apps/web (doesn't resolve solid-js/the tsconfig) — the AUTHORITATIVE typecheck is `pnpm -r typecheck` (now wired into every package's build). GT-004 follow-up GT-004b: PR review/merge UI (Confirm overlay over the GT-003 gate) + nested-dir tree browse (needs gateway multi-segment {path}) + the in-browser GF-6 edit/commit composer.

| GT-005 | Git CLI + MCP surface + SSH durable key-binding (E1.4) | CLI git repo/pr commands → durable edge (gate-blocked merge → clean exit 1); MCP git tools via GitEffectApi (mint→EffectApi→real durable op, HITL+gate+audit); SSH key→principal from durable PG PrincipalStore | INDEP SECURITY verifier: ACCEPT-w/-followups → adversarial attacks on BOTH surfaces could NOT bypass the gate (no policy/green/approval injection flips it; report_checks/set_branch_protection not exposed to CLI/MCP), no tenant escape/traversal, every git mutation routes through mint→EffectApi (revoked denied, git.merge HITL-gated), SSH binding durable+tenant-scoped, token never logged | GREEN (137 result-groups cli/mcp/git/edge/identity + lints + workspace) | (this) |

**SANDBOX-INDEPENDENT GIT DAILY DRIVER COMPLETE** (GT-001..005): durable on-disk storage → real backup+destructive-restore → durable API writes w/ an unbypassable merge-gate → browser UI → CLI/MCP agent surfaces, all verified end-to-end on real git. Two REJECT-level security holes (path-traversal cross-tenant breakout, merge-gate bypass) + a frontend type-gate gap caught & fixed before commit across the track. GT-005 follow-up (non-blocking, defense-in-depth — verifier confirmed NOT a merge bypass): re-assert the `approve_untrusted_ci` capability inside GitEffectApi for `git.endorse_fork_ci` (currently leans on the run-token authority intersection). Remaining: **GT-006** (the raw clone/push WIRE + external-oracle test) — GATED on the CI/sandbox prod-exec track (P-544/545 + RESHAPE-001), per "don't let the hardest subsystem block the useful one."
