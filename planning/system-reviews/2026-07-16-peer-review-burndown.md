...
| 7b | MED→before-live | events | (introduced by H2's fix) panic path acks the broker while its replayable dead-letter set is a volatile/unbounded/undrained in-memory Vec → a panicked handler's effect is lost after a redeploy | **TODO(P1-before-CT-004d.2-live)** — durable DLQ table (own conn, co-commit tx is rolled back) OR don't-ack panic-dead-letters. + LOWs: DLQ Vec unbounded, mr009b schema accumulation, in-memory absorb intra-batch parity, FNV-collision infinite-Retry, generic-handler unwind caveat. |
