...
| 9 | MED | ci-dispatch | `integration_ci_ct004b_trigger_consumer.rs:188` derives event ids deterministically so "redelivery adds no dup events" passes — but prod `OutboxReserveStore` mints FRESH ULIDs → prod WOULD duplicate. Test double stronger than prod; no restart leg. | **DONE (moot via #8)** — traced the prod path: `emit_reserve_events` (consumer.rs:367) emits via `emit_with_id` with DETERMINISTIC ids (`deterministic_uuid("evt:{subject}")` + `check_event_id(run_id, subject)`), NOT fresh ULIDs — the #8 fix. The minted ULID passed to the store is NOT the outbox dedup key. Proven on BOTH prod reserve stores' redelivery legs: `h1_production_outbox_absorb_closes_the_livelock` (real `OutboxReserveStore`: 2× persist → exactly 3 rows, 0 dup) and `chunk4_..._reconverge` (real `CoCommitReserveStore`: crash-window redelivery → 1 run, 3 events). Premise was pre-#8 stale. |
...
| 11 | LOW-MED | storage | stores migrated BETWEEN the colliding cost_event migrations (pre-CT-004m) have no repair path — money ledger to a wrong-shaped table | **DONE** — new `verify_ci_cost_event_shape(pool)` asserts the 10 metering columns + types via `to_regclass` + `pg_attribute` (search-path aware); a missing/mis-typed column → loud `CiCostStoreError::SchemaShapeMismatch`. Wired into BOTH CI mains (ci-dispatch + ci-controlplane) right after the `ci_durable_migrations` apply, fail-loud (exit 1) — a wrong-shaped money table can never be bound/written. Verified live in the ct004m test: correct shape passes; a money-ledger-shaped `ci_cost_event` fails on the absent `region` column. (Core risk already mitigated by the CT-004m rename; this is the defense-in-depth assertion.) |
...
| 13 | LOW | ci-controlplane | cost_id FNV-1a + ON CONFLICT DO NOTHING, no conflict verification (cost_store.rs:313) → a re-delivered settle with different amounts silently drops a unit | **DONE** — added VERIFY-ON-CONFLICT to `settle_in_tx`: when the INSERT absorbs a re-delivery (0 rows), read the recorded (amount, wholesale, markup) back (`SELECT_COST_EVENT_BY_ID_QUERY`) and compare to the incoming; a divergence returns the new loud `CiCostStoreError::AmountDivergence` (never a silent drop — the key is (tenant,run,job,meter), not the amount). Same-amount re-delivery still returns 0 (idempotent). Only SELECT permission needed; INSERT constant unchanged. Verified live: divergence test (refused + recorded amount unchanged) + 313 unit + cost integration green. |
| 17 | LOW-MED latent | web | expand-context is an unwired pipeline (DiffViewer.tsx:51/68/177; getFileLines 0 call sites) | **TODO(P3) — blocker scoped (do NOT remove).** The backend endpoint (`file_lines` durable.rs:1441 + edge route git_durable.rs:2762, TESTED), the dev-edge mock, the `getFileLines` client, and the DiffViewer props/affordance are ALL built + the DiffViewer degrades gracefully (no dead button). The real blocker: the `file-lines/{oid}` endpoint keys on the **blob oid**, but `PrDiffFileVM` carries no per-file blob oid (only `head_oid` = the commit). Wiring = a cross-stack chunk: extend the diff VM (`PrDiffFile::to_json` + `PrDiffFileVM`) with the per-file new-side blob oid, THEN wire `onExpandContext`/`expandedContext` state + the gapKey→(start,end) range math in `diff.tsx`. Not a quick wire; a contract change. |
...
| 20 | LOW | web | dev-contract.mjs:626 cross-repo `repo:"myelin/myelin"` but real edge emits the bare slug → cross-repo rows 404 vs harness, invisible in e2e | **DONE** — traced the real edge: `PrListRowVM.repo = e.repo_slug` = the BARE `scan_repo_slugs` name (git_durable.rs:1124/1086). Fixed all 4 `SEED_PR_ROWS` to `repo:"myelin"` (bare) + added a load-time fixtures-mirror-contract guard that throws if any row's `repo` contains `/`. 13 PR e2e green (incl. cross-repo buckets); dev-contract loads clean. |
| 21a | LOW | web | cross-repo buckets: no pager, count chip shows page size not total (silent truncation) | **DONE** — the chip now shows the TRUE `page.total` (not `items.length`), and a truncation caption ("Showing N of M …") DISCLOSES any shortfall instead of dropping it silently, via the new pure `bucketPageSummary` (`page.total ?? shown`, `truncated = shown<count || next_cursor`). Endpoint returns everything today so nothing shows, but the surface is now honest-by-construction if it starts paginating. Verified: 5 unit tests + typecheck clean + 6 cross-repo e2e green. (A full prev/next pager is deferred until the cross-repo endpoint actually paginates — it has no `next_cursor` today.) |
| 21b | LOW | web | prs/index.tsx:86 renders raw err.message into a hidden DOM text node (no XSS, but against the range's rule) | **DONE** — removed the `<span style="display:none">{err?.message}</span>` leak in `git/repos/[repo]/prs/index.tsx` (+ dropped the now-unused `err` fallback param). The ErrorBoundary already shows a system-blaming static message; no test referenced the hidden node. typecheck clean. (Note: file has 3 PRE-EXISTING a11y lint false-positives at the `ul role="list"`+`list-style:none` Safari workaround — repo-wide lint debt = 13 errors, separate from this finding.) |
...
| 21d | minor a11y | web | review verdict panel (prs/[n]/index.tsx:458) ad-hoc role="dialog", no focus move/trap/Esc, unlike the DS Dialog | **DONE** — replaced the ad-hoc `<div role="dialog">` with the DS `Dialog` (title "Submit review", size sm): focus move/trap + return-focus, Esc + backdrop dismiss, scroll-lock, APG role=dialog+aria-modal — all inherited from the overlay substrate. Preserved the `verdict-panel`/`verdict-*` testids. typecheck clean; 7 pr-overview e2e green incl. the verdict-submit flow. |
