...
| 7 | HIGH | events | dedup mark commits BEFORE handler effect in a SEPARATE tx (consumer.rs:546) → kill-9 between → redelivery deduped → push arms nothing = AT-MOST-ONCE despite exactly-once claims (MR-023b floor named in dedup.rs:92-100) | **DONE `0886f20` (merge of `ff7fd1f`)** — founder-chosen Option A: platform-wide same-tx co-commit (EventHandler gains a tx handle; mark commits AFTER handle, not before → window closed). All ~20 handlers updated (prod handlers take `_tx`; effect via outbox is idempotent). Kill-9 co-commit test 2/2 on live PG. **Verifier note:** the independent adversarial-AGENT pass died on 3 consecutive transient API 529s; orchestrator did the independent verification instead (crux/#8/poison/commit-failure/conn/dyn-Any all read + probed SOUND) — a supplementary agent pass is queued for API recovery (non-blocking; consumer not live in prod). |
