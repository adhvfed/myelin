...
| 7 | HIGH | events | dedup mark commits BEFORE handler effect in a SEPARATE tx (consumer.rs:546) → kill-9 between → redelivery deduped → push arms nothing = AT-MOST-ONCE despite exactly-once claims (MR-023b floor named in dedup.rs:92-100) | **MECHANISM MERGED `0886f20`, but the SUPPLEMENTARY adversarial agent (2026-07-17, after API recovered) found 3 HOLDs my own verification MISSED — REOPENED, fixing forward (consumer not live in prod, no exposure).** H1 HIGH: the outbox commit REJECTS a duplicate event_id (`pgrelay` reject-parity, NOT ON CONFLICT DO NOTHING as #8's doc claimed) → the crash-window re-emit → Err → handler Retry → PERMANENT LIVELOCK; fix = ride the reserve bundle on the co-commit HandlerTx tx (true co-commit, discharges the ci_run floor). H2 HIGH: a PANICKING handler leaks the open co-commit tx to the pool (raw BEGIN/COMMIT, no Drop, no catch_unwind) → next delivery commits the panicked mark+partial effect → MR-023b bug resurrected; fix = sqlx native Transaction (rollback-on-drop) + catch_unwind in deliver(). H3 MED: deterministic ci.check.updated id omits run_id → distinct runs on same (repo,commit,context) collide; fix = seed evt:<run_id>:<subject>. LESSON: merging a load-bearing change on my OWN verification (forced by the API outage) was insufficient — the independent agent caught real bugs. |
