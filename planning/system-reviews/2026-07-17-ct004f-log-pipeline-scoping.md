...
- **Sub-step 4 (persist)** — NEXT. The live `LogPersist` impl: tenant-scoped FORCE-RLS `log_segment`/`log_anchor` writes (bind-param SQL already frozen: `INSERT_LOG_SEGMENT_QUERY`/`UPSERT_LOG_ANCHOR_QUERY`) + drain pointers to the outbox. Live PG + live S3 integration test. Redaction-AGNOSTIC (just persists rows/blobs) → buildable WITHOUT the step-0 founder decision.
