...
| 3.2 | **`register_automation(AutomationRule{matcher, action, run_as, delegation, budget, gates})`** — stateless per-event reflex; may invoke a durable workflow. | Bus | project admins, subsystems | NOT SHIPPED — the former process-local engine was removed; the running agent-trigger product has its own durable store and authenticated surface | Bus §3.5; ADR-19 |
| 3.3 | **`arm_trigger/disarm_trigger(Trigger{owner, condition, arms_subject, on_resolve, stale_after})`** — stateful per-person promise; armed→{resolved\|stale\|disarmed}; `stale_after` is a `myelin-flow` timer. `condition` is a `QueryAst` over projection state ("all `blocked_by` resolved"). | Bus | Issues, users | NOT SHIPPED — the former mutex timer and arming map were removed | Bus §3.6; recon §2/OQ-C |
...
| # | Contract | Owner | Consumed by | Status | Site |
|---|---|---|---|---|---|
| 5.1 | **`ArtifactRef`** — `myelin://<tenant>/<subsystem>/<type>/<id>[#sub]`; `parse`/`format` reject ambiguity. Git ids may preserve a hierarchical repository name (`platform/api`) because the first three scope segments remain fixed; other ids remain one path segment. **Issues id grammar frozen `<PROJECTKEY>-<seqno>`** as the stored canonical key; `#1421` is the render-time display projection (REF-3 reconciliation). | Refs (parse/format/resolve) + Bus (type table) | every service | SHARPENED (Issues key grammar; hierarchical Git ids; REF-3 reconciled) | Refs §3.1; recon §3 |
...
| 9.4 | **Durable signal (multi-day HITL)** — `state=waiting` holds no runtime; an `approval`/`cancel`/`ci.result`/`job.done` signal arrives hours/days later (idempotent), re-leases + replays + consumes. The durable signal substrate ships for hosted-agent HITL and job completion; merge-queue `ci.result`, protected environments, and owner approval-card workflows remain consumers to compose. | Workflow | Agent HITL, future durable workflows | PARTIAL — substrate ships; listed workflow consumers are not all composed | Workflow §3.4; recon §X-1/OQ-F |
...
| 11.3 | **KMS hierarchy** — per-cell root → per-tenant KEK → per-tenant/per-subject DEK. Platform-managed keys are durable and crypto-shreddable. A production key-origin boundary for BYOK/HYOK, including real customer-key calls and Search/Agent admission, remains open; no in-process boolean is accepted as proof that derived stores skipped plaintext. | Storage/GDPR | every encrypted store, Search, Agent | PARTIAL (platform KMS shipped; BYOK/HYOK open) | Storage §4 |
