// Verify the GENERATED tokens.css matches the CANONICAL hand-authored projection on the surface the
// frontend canon names: the semantic color vars across all 3 themes + the z-index scale. This proves
// the SD pipeline reproduces the parameterization model (re-skin = token swap), not a fork.
//
// Exits non-zero on ANY mismatch (a gate, not a report).
...
const CANONICAL = resolve(
  __dirname,
  "../../../../design-planning/08-design-system/01-tokens/tokens.css",
);
...
if (mismatches.length) {
  console.error("token verify FAILED — generated tokens.css diverges from canonical:");
  for (const x of mismatches) console.error("  " + x);
  process.exit(1);
}
console.log(
  `tokens verify: OK — ${SEMANTIC.length} semantic vars x 3 themes + ${ZSCALE.length} z-index vars match canonical.`,
);
