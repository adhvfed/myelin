...
function errorCopy(kind: KnowledgeErrorKind): string {
  ...
  return "We couldn’t confirm the new page. Check the page list before retrying.";
}
...
export function KnowledgeCreateDialog(props: KnowledgeCreateDialogProps) {
  ...
  createEffect(() => {
    ...
    setTitle(""); setTemplate("blank"); setVisibility("private"); setSubmitting(false); setError(null);
  });
  ...
  const submit = async (event: SubmitEvent) => {
    ...
    try {
      const result = await mutate({ op: "create", title: title(), template: template(), visibility: visibility(), clientNonce: crypto.randomUUID() });
  ...
  return (
    <Dialog open={props.open} onClose={close} title="Create a Knowledge page" description="Choose a useful starting point. Every page stays private until you share it with your team." size="lg" dismissable={!submitting()} initialFocus={() => titleInput}
      ...
      <form id="knowledge-create-form" class="knowledge-create-form" onSubmit={submit}>
        <label class="knowledge-field">Page title<input ref={titleInput} value={title()} maxlength={512} autocomplete="off" placeholder="Service ownership model" disabled={submitting()} aria-invalid={Boolean(error())} onInput={(event) => { setTitle(event.currentTarget.value); setError(null); }} /></label>
        <fieldset class="knowledge-template-fieldset"><legend>Start from</legend><div class="knowledge-template-grid"><For each={TEMPLATES}>{(item) => <label class="knowledge-template-card" classList={{ selected: template() === item.id }}><input type="radio" name="template" value={item.id} checked={template() === item.id} onChange={() => setTemplate(item.id)} /><strong>{item.title}</strong><span>{item.copy}</span></label>}</For></div></fieldset>
        <label class="knowledge-field">Visibility<select value={visibility()} onChange={(event) => setVisibility(event.currentTarget.value as KnowledgeVisibility)}><option value="private">Private — only me</option><option value="team">Team — everyone in this workspace</option></select></label>
